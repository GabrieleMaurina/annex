...
- **Purpose:** create a game with default settings: name `Game with <playerName>` unless `name` is given, map `World`, 2 slots, `Supremacy` game mode, `Balanced` blitz, 2 defence dice, `Constant` cards, `Random` placement, entrenchment off, toxins off, portals off, radiation off, starvation off, turn troops off, bounties off, supply lines off, 120s turn duration; make the caller host and move their socket into the room.
- **Content:**
  ```ts
...
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `not the host`, `game already started`, `invalid name`, `invalid map`, `invalid slots`, `invalid banned players`, `invalid team`, `invalid game mode`, `invalid blitz`, `invalid defence dice`, `invalid cards`, `invalid placement`, `invalid fortification`, `invalid entrenchment`, `invalid toxins`, `invalid portals`, `invalid radiation`, `invalid starvation`, `invalid turn troops`, `invalid bounties`, `invalid supply lines`, `invalid turn duration`, `invalid password`, `invalid visibility`, `game name already in use`.

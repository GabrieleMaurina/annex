...
- **Ack:** unlike other calls, this one is not the shared Ack response: it carries the computed probabilities alongside it, the same shape `game:attackSelectEnd` uses:
  ...
  | {
      ok: true;
      game: GameState;
      blitzWinProbabilities: number[]; // index i = probability of winning with i+1 ships via blitz, length = caller's ship count on attackSeaTerritoryId
...
- **Purpose:** resolve a fight between the caller's and `attackSeaDefenderId`'s ships on `attackSeaTerritoryId`, via the same dice-based combat as a land attack (see "Seas and ships" above) except a tie favors neither side. For `type: 'regular'`, `ships` (1 up to `min(caller's ships there, 3)`) fight exactly one exchange via `attackSea()` in `dice.ts`, and the raw dice results are returned (see Ack below) so the client can animate the roll; the defender rolls `min(its ships there, defenceDice)`. For `type: 'blitz'`, `ships` (1 up to the caller's full ship count there — no reservation, since there's no territory to hold) fight to elimination of one side via `trueBlitz()`, `balancedBlitz()`, or `fairBlitz()` (chosen by the game's `blitz` setting, exactly like a land blitz), except every dice exchange along the way still treats a tie as neutral rather than favoring the defender. As with land, `'Fair'` blitz never rolls dice: the losses are fixed in advance from the ship counts and defence dice, the side favoured by the exact odds wins, and the loser is wiped. Losses are applied immediately to both sides' ship counts; a side reaching `0` ships there is removed from `seas` entirely. If either side is left with `0` ships there, `attackSeaTerritoryId`/`attackSeaDefenderId` reset to `null`, ending this fight (same "no ships left to continue" rule fortify/attack territories follow); otherwise both stay set and blitz win probabilities are recomputed for the reduced ship counts, mirroring `game:attack`. Unlike `game:attack`, there is no conquest and no follow-up move. The caller must be the player at `turnPlayerIndex`, the game must be `playing` and in `'attack'` with both selected, and the defender must still have at least 1 ship there.
- **Content:**
  ```ts
...
- **Ack:** like `game:attack`, not the shared Ack response: it carries fresh blitz win probabilities (empty when either side was left with `0` ships) plus the dice actually rolled (both empty for `type: 'blitz'`, since blitz doesn't animate a roll):
  ...
  | { ok: false; error: string }
  ```
  Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not attack phase`, `no attack selection`, `invalid attack type`, `invalid ships`, `invalid defender`.

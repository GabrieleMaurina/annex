...
Every socket starts in the `home` room on connect. A player moves to a game's room (`game:create` / `game:join`) and can be moved back to `home` (kicked, or `player:identify` reporting `room: 'home'`). A client running a local offline game instead identifies with `room: 'offline'`: the server keeps its identity and display name up to date (so `player:setName` keeps working) but sends it no `home:games` and puts it in no game; the offline game itself is played entirely client-side and never touches the server.
...
- **`lobby`**: no bot takeover here; that's a `playing`-only concept (see "Bots" below). A disconnecting spectator simply stays queued, not connected. A disconnecting player who holds a slot gets stood in for by the front *connected* spectator in the queue, if any (see "Slot stand-ins" below); with no connected spectator waiting, the player is instead removed from the game outright and their slot freed (open for another `game:join`, or for the host to fill with `game:addBot`). A mismatched-room `player:identify` (see below) is a full, permanent departure exactly as a no-stand-in disconnect is: clicking away or joining a different game removes the player entirely (cementing any stand-in currently holding their slot for good), with no path back. Reconnecting *before* the game starts (a fresh `game:join`) reseats a removed player as a player again if a slot is open (restoring their original `team` and `color` when nothing has since claimed that `color`) and their host-priority position is untouched throughout, since it was never lost; if no slot is open by then, they join as a spectator like anyone else. A player whose slot is currently held by a stand-in instead reclaims it via a room-matching `player:identify` (see "Slot stand-ins" below).
- **`playing`**: slot, territories, and troops stay exactly as they were; only the socket drops. Reconnecting with the same `key` (any device) silently resumes that slot, instantly restoring human control. In the meantime the seat doesn't just idle out its turns: it is immediately taken over by a bot (see "Bots" below), so the game keeps moving while the player is away. A surrendered player is the exception: since they can no longer act, a mismatched-room `player:identify` (e.g. clicking the game's "Leave" button) clears their game membership so the client can navigate elsewhere, same as the `ended` case below; the seat itself stays untouched.
...
### `player:identify`
- **When sent:** once, immediately after connecting; re-sent on every reconnect (new tab, reload, dropped connection); and re-sent whenever the client's own declared room changes, e.g. after creating/joining a game, navigating back to `/`, navigating directly to a different game's URL, or entering/leaving a local offline game.
- **Purpose:** register this socket against a persistent player identity, and declare which room the client believes it's in: `'home'`, a game name, or `'offline'` (client is running a local offline game). The server uses this to place the socket correctly and to detect stale membership: if `room` doesn't match its record (e.g. another tab is still in a game but this one reports `'home'`), it updates their game membership accordingly (see "Leaving and reconnecting" above). `room: 'offline'` is treated like `'home'` for membership purposes (any stale game seat is left) but additionally suppresses `home:games` for this socket. A reconnect also re-checks `hostId` for whatever game the player is in, in case a former host is due back (see `hostId` above).
- **Content:**
  ```ts
...
- **Purpose:** create a game with default settings: name `Game with <playerName>` unless `name` is given, map `World`, 2 slots, `Supremacy` game mode, `Balanced` blitz, 2 defence dice, `Constant` cards, `Random` placement, entrenchments off, toxins off, portals off, radiations off, starvation off, turn troops off, bounties off, supply lines off, fog of war off, alliances off, random disconnect bot difficulty and personality (see "Bots" below), 120s turn duration; make the caller host and move their socket into the room.
- **Content:**
  ```ts
...
- **Purpose:** remove `botPlayerId` from the game entirely, freeing its slot, and permanently release the bot's identity (unlike removing a human, there's no `id`/`key` left behind to ever reconnect as). The caller must be host, the game must still be `lobby`, and `botPlayerId` must currently be a bot seated in this game.
- **Content:**
  ```ts
  { botPlayerId: number }
  ```
...
### `home:games`
- **When sent:** event-driven, not polled: to every socket currently in the `home` room, whenever a change could affect the list; a game created, destroyed, or ended, or a `game:join`/`game:settings`/leave/reconnect changing a public game's roster, name, map, slots, host, or password. Also sent individually to a socket the instant its own `player:identify` puts it in the `home` room, so it has the current list without waiting for the next change. A socket that identified with `room: 'offline'` is skipped entirely until it next identifies as `'home'`.

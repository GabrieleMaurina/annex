...
### `GET /games/history`
- **When sent:** the "Games" page (`/games`) loads and on every filter / page change (debounced for the free-text search). One request per rendered table page — the replay and its map are separate requests (below).
- **Query params** (all optional): `page` (1-based, default 1), `pageSize` (default 20, max 100), `search` (case-insensitive substring, matched against map name, game mode, or any player name), `playersMin` / `playersMax` (inclusive range on the game's player count, done via `$expr` on `$size` of `players` so it needs no denormalised field), `mode` (an exact game-mode name), `mapName` (exact — the UI offers only the built-in map names), any **game-setting** key (`blitz`, `defenceDice`, `cards`, `placement`, `fortification`, `entrenchments`, `toxins`, `portals`, `radiations`, `starvation`, `roundTroops`, `bounties`, `supplyLines`, `fogOfWar`, `alliances`, `turnDuration`, `disconnectBot*`) taking one of that setting's own values (each an exact `settings.<key>` match), `minRounds` / `maxRounds` (inclusive range on rounds played), `outcome` (`won` / `lost`), `positionMin` / `positionMax` (1-based inclusive range over the caller's finishing position — "position" throughout, since "placement" is the troop-setup game setting), `sort` (`newest`, `rounds`, or `position`) with `sortDir` (`asc` / `desc`, default `desc`) — the "Games" UI exposes these as six one-shot options (Newest / Oldest / Most rounds / Fewest rounds / Best position / Worst position), `mine` (`1` to restrict to the caller's own account). Only `mine`, `outcome`, `position*`, and `sort=position` consult the session (via the `anx` cookie); everything else is identity-free. The caller-relative fields (finishing position, whether they won) are computed in the aggregation from the game's `results` / `winnerIds` matched to the caller's `players[].playerId`, so they work on every stored game regardless of whether the newer denormalised `players[].rank`/`won` are present.
- **Response:** `{ games: Row[]; total: number; page: number; pageSize: number }` where `Row` is `{ id, name, mapName, gameMode, roundNumber, playerCount, winnerIds, winnerNames, yourRank, settings, players: { name, isBot, color, team }[] }`. `yourRank` is filled from the session when logged in (regardless of `mine`), else `null`. `id` opens the replay at `/games/replay/<id>`.
- **No access restriction:** any caller may page and filter across **every** stored game, not just their own — the UI simply defaults `mine` on for a logged-in user. `mine=1` from an anonymous caller returns an empty page. A game is recorded on end only if at least one full round completed (`roundNumber >= 1`, same gate as ELO and the `maps` archive); an **offline** game is never sent to the server or persisted anywhere, so it never appears here (see `game:stored` below for how its review works instead).
...
- **Response:** the full stored game document (see the `games` collection below), or `404 { ok: false, error: 'not found' }`. Public and unrestricted — any id, any caller; like `GET /games` it runs ahead of the identity middleware, reads no cookie, and never mints or slides a session. (An offline game is never requested here — it has no id and no stored document; see `game:stored` below.)
...
### `game:stored`
- **When sent:** once, to every socket in a game's room, after the finished game has been written to the `games` collection (see `GET /games/history` above) — only for games that completed at least one full round. Never sent for an **offline** game (there's no server to write to) and not resent on reconnect; a client that missed it can still reach the replay from the "Games" page.
- **Purpose:** every still-connected client (players and spectators) navigates to the standalone replay page (`/games/replay/<gameId>`) — the end-of-game view **is** the review view: map replay + chat + emojis + results table, all from the server. Until it arrives (or for an offline game, always) the client shows an in-place end screen (results table, with a button to watch the replay from live memory — `game:replay`, see above). An offline game is forgotten the moment the player leaves it: nothing is persisted, so there's no page or URL to return to afterward.
- **Content:**
  ```ts
  { gameId: string }
  ```

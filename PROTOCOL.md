...
During `'attack'`, a player with ships in a sea that also holds another player's ships may fight them (`game:attackSeaSelectStart`/`game:attackSeaSelectDefender`/`game:attackSea` below): the same dice-based combat as a land attack (attacker rolls up to `3` dice for however many ships it commits to this roll, defender rolls up to `defenceDice`, pairing largest to largest), with two differences — a tie favors neither side (no loss either way, rather than the defender winning), and the attacker may commit every ship it has there, none need stay uncommitted. This is purely a fight over ships within one sea, never across two; there is no equivalent of blitz, conquest, or a follow-up move for it. `'attack'` is only auto-skipped once neither a land attack nor a ship fight (nor a ready nuke) is available.
...
- **Purpose:** set `attackSeaDefenderId`. `defenderId` must be a different player with at least 1 ship on `attackSeaTerritoryId`. The caller must be the player at `turnPlayerIndex`, and the game must be `playing` and in `'attack'`.
- **Content:**
  ```ts
  { defenderId: number }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not attack phase`, `no sea selected`, `invalid defender`.

### `game:attackSea`
- **When sent:** the player whose turn it currently is, during `'attack'` with both `attackSeaTerritoryId` and `attackSeaDefenderId` set, confirms a ship-fight roll.
- **Purpose:** resolve one exchange between the caller's and `attackSeaDefenderId`'s ships on `attackSeaTerritoryId`, via the same dice-based combat as a land attack (see "Seas and ships" above) except a tie favors neither side and the caller may commit every ship it has there. `ships` (1 up to `min(caller's ships there, 3)`) are committed; the defender rolls `min(its ships there, defenceDice)`. Losses are applied immediately to both sides' ship counts; a side reaching `0` ships there is removed from `seas` entirely. If either side is left with `0` ships there, `attackSeaTerritoryId`/`attackSeaDefenderId` reset to `null`, ending this fight (same "no ships left to continue" rule fortify/attack territories follow). Unlike `game:attack`, there is no conquest, no follow-up move, and no blitz variant. The caller must be the player at `turnPlayerIndex`, the game must be `playing` and in `'attack'` with both selected, and the defender must still have at least 1 ship there.
- **Content:**
  ```ts
  { ships: number }
  ```
- **Ack:** like `game:attack`, not the shared Ack response: it carries the dice actually rolled:
  ```ts
  | { ok: true; game: GameState; attackerDice: number[]; defenderDice: number[] }
  | { ok: false; error: string }
  ```
  Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not attack phase`, `no attack selection`, `invalid ships`, `invalid defender`.
...
- **Purpose:** let every client play the attack sound/dice animation for the ship fight, since `GameState` is a snapshot with nothing to infer this from. `attackingShips`/`defendingShips` are the ships each side committed to this exchange (before losses); `attackLosses`/`defenceLosses` are what each side lost.
- **Content:**
  ```ts
  { seaTerritoryId: number; attackerId: number; defenderId: number; attackingShips: number; attackLosses: number; defendingShips: number; defenceLosses: number }
  ```

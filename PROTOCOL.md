...
- **Response:** `{ name, territories, seaTerritories, bonuses, image, imageMime }` (`image` base64) — the archived map geometry and picture from the `replay_maps` collection, so the replay page can render the board without the geometry ever being embedded in the game document. `404 { ok: false, error: 'not found' }` if unknown. Public and unrestricted — any id, any caller; runs ahead of the identity middleware and touches no cookie.
...
- **`GET /player-maps/:id`**: the full map for the editor or for play, `{ id, name, authorId, territories, seaTerritories, bonuses, image (data URL), imageMime, generation, dangerous, likeCount, mine, createdAt }`. `404` if unknown, or if `dangerous` and the caller is not the author.
...
- **`POST /player-maps`**: create (logged in). Body `{ name (1–40 chars, unique per author), territories, seaTerritories, bonuses, image (data URL, ≤6 MB, png/jpeg/webp), generation }` where `generation` is `{ seed (1–20 chars), size, type, fill, seas }` or `null`/omitted. Geometry is validated server-side (ids unique across `territories` and `seaTerritories` together, neighbours valid and mutual, the whole graph — land and sea — connected, at most 1200 territories total (`MAX_CONTINENTS` x `MAX_CONTINENT_SIZE`, land + sea combined), 1–30 continents each sized 2–40 land territories, bonuses 2–25), the `generation` enums are checked, then the image is moderated. Response `{ ok: true, id } | { ok: false, error }` where `error` is one of `'not logged in'`, `'too many requests'`, `'invalid name'`, `'name taken'` (another of the caller's maps already has that name), `'invalid territories'`, `'invalid seas'`, `'invalid bonuses'`, `'not enough territories'`, `'too many territories'`, `'duplicate territory id'`, `'invalid continent'`, `'invalid neighbor'`, `'asymmetric neighbor'`, `'territories not all connected'`, `'invalid continent size'`, `'invalid generation'`, `'invalid image'`, `'image rejected'`, `'moderation failed'`, `'server error'`.

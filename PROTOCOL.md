...
```
`radiationUpcoming`, `hands`, `playerStates`, and `turnPhase` are full snapshots taken at the moment of the frame (all cheap, unlike `territories`), so a replay's scoreboard can track card hands, troop counts, nuke arsenals, per-player elimination/surrender, and the round/phase as they evolve. Changes that aren't themselves a player action on the board — building or completing a nuke, a surrender, an elimination by radiation — get no frame of their own; they surface in whichever `ReplayFrame` comes next, the same rule `toxinTerritories` and `radiationTerritories` already follow. A replay client shows the acting player's (`playerId`) hand and nuke arsenal, since those are per-player.
...
- **`POST /player-maps`**: create (logged in). Body `{ name (1–40 chars, unique per author), territories, bonuses, image (data URL, ≤6 MB, png/jpeg/webp), generation }` where `generation` is `{ seed (1–20 chars), size, type, fill }` or `null`/omitted. Geometry is validated server-side (ids unique, neighbours valid and mutual, all territories connected, at most 1200 territories total (`MAX_CONTINENTS` x `MAX_CONTINENT_SIZE`), 1–30 continents each sized 2–40, bonuses 2–25), the `generation` enums are checked, then the image is moderated. Response `{ ok: true, id } | { ok: false, error }` where `error` is one of `'not logged in'`, `'too many requests'`, `'invalid name'`, `'name taken'` (another of the caller's maps already has that name), `'invalid territories'`, `'invalid bonuses'`, `'not enough territories'`, `'too many territories'`, `'duplicate territory id'`, `'invalid continent'`, `'invalid neighbor'`, `'asymmetric neighbor'`, `'territories not all connected'`, `'invalid continent size'`, `'invalid generation'`, `'invalid image'`, `'image rejected'`, `'moderation failed'`, `'server error'`.
...
- **Content:**
  ```ts
  {
    ...
    fill: 'full' | 'mixed' | 'sparse';               // playable-area share; sparse leaves the most water / inaccessible space
  }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `not the host`, `game already started`, `invalid seed`, `invalid size`, `invalid type`, `invalid fill`, `server error`.

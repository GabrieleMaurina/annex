...
### `auth:confirmEmail`
- **When sent:** the `/email_confirmation/<code>` client page loads and opens a socket (the socket must already be identified).
- **Content:** `{ code: string }`
- **Purpose:** mark the account's `validated_email` true, delete the confirmation, and **log this socket in** (same promotion as `auth:login` below, including the `gameName` redirect).
- **Ack:** same shape as `auth:login` — `{ ok: true; username: string; clientSettings; gameSettings; gameName: string | null } | { ok: false; error: string }`. Errors: `invalid code`, `invalid or expired code`, `not identified`, `too many requests`, `server error`.
...
### `auth:login`
- **When sent:** a client submits the login form (from `home`, a lobby, a `playing` game's players panel, or the results screen). The socket must already be identified.

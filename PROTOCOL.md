...
### `auth:resetPassword`
- **When sent:** the `/password_reset/<code>` client page submits a new password (the socket must already be identified).
- **Content:** `{ code: string; password: string; stayLoggedIn?: boolean }` — password 8–128 chars; `stayLoggedIn` defaults true (see `auth:login`).
- **Purpose:** set the new password hash, mark `validated_email` true (clicking the emailed link proves ownership), delete the reset code, delete every existing `sessions` row for that account, then **log this socket in** (same promotion as `auth:login`, including the `gameName` redirect).
- **Ack:** same shape as `auth:login` (with `gameName`). Errors: `invalid code`, `invalid password`, `invalid or expired code`, `not identified`, `too many requests`, `server error`.

...
- **`POST /player-maps/:id/report`**: record a distinct report from the caller (logged in). At three distinct reporters the map is marked `dangerous`. `{ ok: false, error }` where `error` is `'map not found'` (unknown id, or already `dangerous`), `'cannot report yourself'` (the caller is the map's author), or `'already reported'` on a repeat.

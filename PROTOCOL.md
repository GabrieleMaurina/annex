...
Three settings blobs travel in the `GET /session` response (client and game settings also in the login/logout responses): **client settings** (`muted`, `volume`, `animationsDisabled`), **game settings** (the remembered lobby setup: `mapName`, `mapGeneration`, `playerMapId` (a `player_maps` id string or `null`, the remembered library map, re-selected via `game:selectPlayerMap` when the client next creates a game, and losing out to `mapGeneration` when both are set), `slots`, `bots` (`{ difficulty, personality }[]`, each field a bot-profile value or `'random'` — the lobby's bot seats, re-added when the client next creates a game, online or offline), `localPlayers` (`string[]` — the names of the extra hot-seat seats in an offline lobby; re-added only when the next game the client creates is offline, since online games never hold more than one local player), and every `game:settings` rule field — used as the defaults when the client next creates a game), and **home filters** (the remembered home-screen game search: `players` (`{ id, label }[]`, so the chips render without re-resolving names), `name`, `mode`, `mapName`, `mapGenerationSize`, `mapGenerationType`, `mapGenerationFill`, `playersMin`/`playersMax`, `roundsMin`/`roundsMax`, `phase`, `password`, `hasBots`, `settings` (per-rule filter map), `sort` — used as the defaults for the home game list). For a logged-in client all three are persisted in the account and saved via `PATCH /settings` (debounced). For an anonymous client nothing is persisted: it always receives the server defaults and its `PATCH /settings` calls are ignored.
...
A bot is a seated player like any other (counted in `players`, never `spectators`) with `isBot: true` and a fixed `botDifficulty`/`botPersonality` (`null`/`null` for a real, non-bot player). A bot whose difficulty and/or personality was chosen as `'random'` (see `game:addBot`, `game:setBotProfile`, and the disconnect takeover below) really plays with a concrete value, but `GameState.players` reports `'random'` for that field, and its `name` shows `Random`, until the game ends (`state` becomes `'ended'`), at which point the concrete values are revealed and a `name` still derived from the profile is updated to match. It acts on its own turns the same way a human would, calling the same `game:*` actions, subject to the same rules and validation; there is no bot-only shortcut through the protocol. `botDifficulty: 'idle'` is the one special case: an idle bot never acts at all, instead force-ending its whole turn from wherever it currently sits the instant it comes up; the same force-advance `turnDuration` timing out triggers (see `turnDuration` above), except any `'deploy'`-phase troops it hasn't placed yet are simply discarded rather than dropped at random, since an idle bot never even attempts to act. A bot can never be offered an alliance (`game:offerAlliance` above) or sent a *targeted* `game:sendEmoji` (an emoji sent to everyone still reaches it, since a bot receives `game:state`/broadcasts like anyone else; it just never acts on them).
...
**GameSummary** (the rows of `GET /games/live`'s page)
```ts
{
  name: string;
  mapName: string; // equal to `GameState.mapName`: a player map's name, or a generated map's `<Type> (<Fill>-<Size>)` label (e.g. "Terrain (Sparse-Large)"); "" while a lobby game has no map yet
  mapGeneration: { size: MapSize; type: GenerationType; fill: Fill } | null; // non-null iff the map is generated; seed omitted
  hostName: string;
  playerCount: number;
  slots: number;
  ...
  hasPassword: boolean;
  hasBots: boolean;    // whether any seated player is a bot
  createdAt: number;   // epoch ms the game was created (drives the "Newest"/"Oldest" sort)
...
```ts
{
  ...
  radiationUpcoming: number[];                 // GameState.radiationUpcomingTerritoryIds at this frame
  seaShips: { seaTerritoryId: number; playerId: number; ships: number }[]; // flat, one entry per (sea, player) holding ships at this frame; empty on a map with no seas or before any ship exists
  hands: { playerId: number; cards: { territoryId: number | null; symbol: string | null }[] }[];
  playerStates: {                              // one entry per player, full snapshot at this frame
    playerId: number;
    eliminated: boolean;                       // in GameState terms: out of the game as of this frame
    surrendered: boolean;
    killedPlayerIds: number[];                 // players this one has eliminated (bounty count)
    nukes: number;                             // arsenal, 0 unless the nukes setting is 'on'
    antiNukes: number;
    nukeProjects: { kind: 'nuke' | 'antiNuke'; installmentsPaid: number; lastPaidRound: number }[];
  }[];
  turnPhase: 'territory' | 'troop' | 'capital' | 'deploy' | 'sail' | 'attack' | 'fortify' | 'entrench' | 'toxins';
  animation:
    ...
    | { type: 'nuke'; fromTerritoryId: number; targetTerritoryId: number; intercepted: boolean; interceptFromTerritoryId: number | null; playerId: number }
    | { type: 'buyShips'; sourceTerritoryId: number; seaTerritoryId: number; ships: number; fromPool: boolean; playerId: number }
    | { type: 'sail'; fromSeaTerritoryId: number; toSeaTerritoryId: number; ships: number; playerId: number }
    | { type: 'attackSea'; seaTerritoryId: number; attackerId: number; defenderId: number; attackLosses: number; defenceLosses: number };
  roundNumber: number;
  playerId: number;
}
```
`radiationUpcoming`, `hands`, `playerStates`, `turnPhase`, and `seaShips` are full snapshots taken at the moment of the frame (all cheap, unlike `territories`), so a replay's scoreboard can track card hands, troop counts, nuke arsenals, per-player elimination/surrender, ships, and the round/phase as they evolve. Changes that aren't themselves a player action on the board — building or completing a nuke, a surrender, an elimination by radiation — get no frame of their own; they surface in whichever `ReplayFrame` comes next, the same rule `toxinTerritories` and `radiationTerritories` already follow. A replay client shows the acting player's (`playerId`) hand and nuke arsenal, since those are per-player.
`territories` is a full snapshot (not a diff) of every territory's owner, troop count, and entrenchment turns remaining immediately after the change described by `animation`: `isCapital` isn't included, since it's a static property of the territory (see `turnPhase` above) that a client already has from `GameState.territories`, but `entrenchedTurns` is included since, unlike `isCapital`, it changes over the course of the game and a replay does depict it (both the octagon and the entrench animation, see `game:entrenched` below). `toxinTerritories` is a parallel, independent snapshot of every currently-toxined territory (it can't live inside `territories`, since a toxined territory has no owner and so is never in that array at all; see the `toxins` paragraph below), same shape as `GameState.toxinTerritories`. `radiationTerritories` is the same idea for radiation (see the `radiations` paragraph below), same shape as `GameState.radiationTerritoryIds`; unlike `toxinTerritories` it isn't empty at the start of a replay, since radiation (when enabled) exists from `game:start` rather than being placed by a player action; the replay's starting point (index `0`, see `game:replay` below) uses `initialRadiation` for it instead of an empty array. `seaShips` is the same idea for ships (see "Seas and ships" above), same flat shape as `GameState.seas` would produce if flattened to one entry per (sea, player) pair instead of grouped by sea; it's always empty at the start of a replay, since ships can't exist before any `game:buyShips` call. A game's replay is the sequence of these snapshots: the state right after `game:start` deals territories (no frame: nothing preceded it, and `toxinTerritories` starts empty since toxins can't exist before any `game:toxins` call), then one `ReplayFrame` for every subsequent troop deployment, fortify, attack, entrenchment, toxin placement, or starvation loss for the rest of the game, in order. `'fortify'`-type frames cover both an actual `game:fortify` move and troops moving into a just-conquered territory (`game:attackMove`, or the turn timer completing either); a `'deploy'`-type frame covers `game:deploy`, a capital's placement troops, a card set's per-territory bonus, and the turn timer dropping leftover troops; a `'entrench'`-type frame covers a `game:entrench` call, reflecting the troops consumed off the board; a `'starve'`-type frame covers troops removed by `starvation` (see above) at the end of a player's turn; a `'toxins'`-type frame covers a `game:toxins` call; a `'buyShips'`-type frame covers a `game:buyShips` call; a `'sail'`-type frame covers a `game:sail` move or the turn timer completing one; an `'attackSea'`-type frame covers a `game:attackSea` roll; one frame per territory actually touched, not per individual troop unit, even when several troops land on (or are removed from) the same territory in one go. A temporary toxin expiring produces no frame of its own (unlike every other `toxinTerritories`/`territories` change, it isn't the direct result of a player action): it's only reflected in whichever `ReplayFrame` happens to come next, whose `toxinTerritories` snapshot simply no longer includes it, exactly as `game:toxinExpired` (see below) reflects it live without a dedicated replay animation to match. Radiation moving or expanding follows the same rule for the same reason (it's never a player action either): it produces no frame of its own, and is only reflected in whichever `ReplayFrame`'s `radiationTerritories` snapshot happens to come next, exactly as `game:radiationChanged` (see below) reflects it live. A `'nuke'` frame covers a `game:launchNuke` call (landed or intercepted); its `territories` snapshot reflects the obliteration, and its `mapDelta` in the stored (`GET /games/replay/:id`) form carries `ownerId: -1` for the tile that left `territoryOwners` (the same removal marker used for radiation/toxin removals), which a replay client applies by dropping the tile rather than reassigning it. A successful attack that conquers its target always produces two separate, consecutive frames: the `'attack'` frame for the battle itself (troop losses and, if conquered, the ownership transfer), immediately followed by a `'fortify'` frame for however the resulting troop move happens: a later `game:attackMove`, or, if the attacker's hand hit 5+ cards or the conquest ended the game outright, one resolved automatically within the same turn. This still holds for a free-conquest attack against an unowned, non-toxined territory (see the `toxins` paragraph and `game:attack` below): `defenderId` on the `'attack'` frame is `undefined` in that case, since there was no real defending player. `roundNumber` is `GameState.roundNumber`'s value at the moment this frame happened (see `turnPlayerIndex` above for what it counts); `playerId` is whoever acted: the depositing/fortifying/entrenching/toxining/starving player, or the attacker for an `'attack'` frame.

**GameState** (used in `game:state` and as the `game` field of every ack below)
```ts
{
  name: string;
  mapName: string; // display-ready: a player map's name, or a generated map's `<Type> (<Fill>-<Size>)` label (e.g. "Terrain (Sparse-Large)") - not unique across generated or player maps; "" while a newly created lobby game has no map (the host must Generate or pick from the library before `game:start`)
  mapGeneration: { seed: string; size: 'small' | 'medium' | 'large' | 'xlarge'; type: 'terrain' | 'dungeon' | 'temple'; fill: 'full' | 'mixed' | 'sparse'; seas: boolean } | null; // the `game:generateMap` inputs that produced the current map, or null when the map is a player map or none is selected; lets a client persist a "use this map" preference (and rebuild the map) without storing the geometry
  playerMapId: string | null; // the `player_maps` document id when the current map is a player map (see `game:selectPlayerMap` / the Map library section), else null; `mapGeneration` is null in that case. Lets a client persist a "use this map" preference and re-fetch the geometry/image, which `GameState` never carries
  slots: number;
  hostId: number;
  ...
  nukes: 'off' | 'on';
  arsenal: { nukes: number; antiNukes: number }; // this recipient's finished devices only
  nukeProjects: { kind: 'nuke' | 'antiNuke'; installmentsPaid: number; lastPaidRound: number }[]; // this recipient's in-progress projects only
  antiNukeTerritoryIds: number[]; // only this recipient's own deployed anti-nukes
  ...
  seas: { id: number; ships: { playerId: number; ships: number }[] }[]; // one entry per sea territory on the map (see "Seas and ships" below); empty on a map with no seas
  winnerIds: number[];
  finalRanking: number[];
...
```
`radiationTerritoryIds` and `radiationUpcomingTerritoryIds` are described under `radiations` below. `nukes`, `arsenal`, `nukeProjects`, and `antiNukeTerritoryIds` are described under `nukes` below; the last three are always the recipient's own only, never another player's, and this holds regardless of `fogOfWar`.

A `players[]` entry carries only what's needed live: current standing (`territoryCount`, `troopCount`, `capitalCount`, `troopsRemaining`, `cardCount`) and status (`connected`, `surrendered`, `eliminated`). `playersKilled`, `troopsKilled`, and `troopsLost` (the player's running troop kill/loss totals) also live here, since `bounties: 'on'` shows a live kill count during play (see `bounties` below). `isBot`/`botDifficulty`/`botPersonality` are described under "Bots" above. A player's cumulative lifetime stats for the just-finished game (troops gained/killed/lost, territories/capitals conquered/lost, cards gained, sets played, turns played) are never part of `GameState` at all: they're irrelevant until the results table after the game ends, so they're pushed once via `game:results` instead of riding along on every `game:state` push for the whole game (see below).

`fogOfWar` is described in its own paragraph further below, alongside `supplyLines`. Unlike every other field on `GameState`, once `fogOfWar` is `'on'` and in effect for a given recipient, `GameState` itself is no longer one single ground truth shared by everyone in the room: `game:state` (and the `game` field of every ack) is computed separately per recipient, and `territories`/`toxinTerritories`/`portalTerritoryIds`/`radiationTerritoryIds`/`radiationUpcomingTerritoryIds`/`seas` are all filtered down to only the ids that recipient can currently see (an unlisted territory is indistinguishable on the wire from one that's simply unclaimed, or from one that isn't a portal/radiated; the client tells territories apart from unclaimed ones using `visibleTerritoryIds`, see below). `selectedTerritoryId` is nulled out for a recipient who can't see the territory it references. `fortifyStartTerritoryId`/`fortifyEndTerritoryId` and `attackStartTerritoryId`/`attackEndTerritoryId` are each treated as a pair: both stay populated as long as the recipient can see *either* endpoint (so the client always has both ids needed to draw the preview arrow between them, fading it toward whichever end it can't see, the same way it fades the arrow for `game:attacked`/`game:fortified`/`game:attackMoved` below), and both are nulled together only when the recipient can see neither. `attackConquestMinTroops` is nulled independently of that pair, whenever `attackEndTerritoryId`'s own territory specifically isn't visible to the recipient (it's the pending conquest-move amount, tied to the target territory alone). `fortifyPathTerritoryIds` is the actual chain of territory ids between `fortifyStartTerritoryId` and `fortifyEndTerritoryId` under `fortification: 'Connected'` (which may pass through ship-held seas, see "Seas and ships" below; just the two endpoints under `'Neighboring'`/`'Unrestricted'`), computed server-side (a recipient can never reconstruct it themselves from `territories` alone, since fog can hide the ownership of territories in the middle of the chain) and split into separate runs wherever a hop's two endpoints are *both* invisible to the recipient, so a client never learns that two territories it can't see are connected by the same fortify chain; each remaining hop keeps at least one visible endpoint, same rule as the pair fields above. It's `[]` whenever no fortify selection is active, or once redaction leaves nothing a recipient can see. `attackPathTerritoryIds` and `sailPathTerritoryIds` are the same idea for `attackStartTerritoryId`/`attackEndTerritoryId` and `sailStartTerritoryId`/`sailEndTerritoryId` respectively: computed server-side (`attackFullPath` for attack, capped at a single sea-bridge hop, see "Seas and ships" below; `sailFullPath` for sail, a sea-to-sea BFS), redacted into runs the same way, and `[]` under the same conditions (no selection active, or nothing left visible after redaction). `visibleTerritoryIds` itself is present only when fog of war is currently restricting this recipient's view; it's the complete set of territory and sea ids they can currently see (their own territories plus every territory directly adjacent to one of them, portal edges included when active, plus every sea they currently hold ships in — ships count as "owning" a sea for this purpose exactly as a land territory's owner does — plus every territory or sea directly adjacent to one of those seas; so a recipient who owns a portal always sees every other portal, however far away, since owning one makes every other portal a direct neighbor for this purpose), and its absence means this recipient's view is unrestricted (fog of war off, game not `playing`, still in the `'territory'`/`'troop'` phase, or this recipient is a spectator, eliminated, or surrendered). Every other field, including every `players[]` entry's status fields (`connected`, `surrendered`, `eliminated`), is always the same for every recipient regardless of fog of war, with one exception: a `players[]` entry's own `territoryCount`/`troopCount`/`playersKilled`/`troopsKilled`/`troopsLost` are nulled to `null` for every player other than the recipient themselves whenever fog of war is restricting that recipient's view (a recipient always sees their own row's numbers).
...
`'deploy'` is the one phase `game:nextPhase` cannot advance (`cannot skip deploy phase`); it only ends once the current player has placed every troop available that turn, via `game:deploy` (see below). Entering `'deploy'` (turn start, or the timer force-advancing from the last phase) silently grants the player a troop pool: `max(3, floor(territoryCount / 3))`, plus, for every continent where they own every one of its territories that still has an owner and isn't toxined or radiated (a nuked, toxined, or radiated tile is skipped, the same way the `Continent` win check skips toxined/radiated ones; a continent left with no such tile grants nothing), that continent's entry in the map's `bonuses` array, plus, in `'Capitals'`, `2` for every capital the player currently controls (always at least their own, more if they've captured others'). `troopsToDeploy` is that pool, decremented as the player calls `game:deploy`; it sits at `0` outside `'deploy'` and in the `lobby` state.

`'sail'`, `'attack'`, `'fortify'`, `'entrench'`, and `'toxins'` are each skipped automatically, the instant they'd otherwise begin, if the player has no legal move in them: `'sail'` needs some sea holding at least one of the player's ships with another sea reachable from it (and is skipped outright on a map with no seas); `'attack'` needs some owned territory with more than `1` troop next to an enemy one or a free-conquest territory (see the `toxins` paragraph below), or a ship fight available in some sea (see "Seas and ships" above); `'fortify'` needs some owned territory with more than `1` troop next to another territory the same player owns (under `'Neighboring'`), or able to reach one through a chain of owned territories that may also cross ship-held seas (under `'Connected'`, see "Seas and ships" below), or, under `'Unrestricted'`, simply owning more than one territory at all; `'entrench'` needs some owned, non-capital territory with at least `2` troops (and is skipped outright whenever `entrenchments` is `'off'`, regardless of troops); `'toxins'` needs the player to own more than one territory (toxining a player's last territory would eliminate them, which is never allowed) and, among all but their last, some owned, non-capital territory with enough troops to afford the current toxin cost whose removal wouldn't split the map (see the `toxins` paragraph below), and is skipped outright whenever `toxins` is `'off'`. This is re-checked (and skipped past again if it still doesn't hold) every time entering the phase would otherwise happen: after `'deploy'` ends, after every `game:attack`/`game:attackSea` that doesn't leave a conquest or ship fight pending, after every `game:attackMove`, after every `game:entrench`, after every `game:toxins`, and after `game:sail`/`game:fortify`/timeout would normally hand off to the next phase, so a player can never be left stuck in a phase with nothing legal to do in it. Clients are expected to mirror this check locally (independently of whatever the server has already done) and call `game:nextPhase` themselves the moment they detect it, as a redundant safety net rather than the source of truth; the server's own skip always wins.
...
A map may include **seas** — neutral, unowned territories (`GameMap`'s `seaTerritories`, see the Map library section and `game:mapGenerated` below) that sit in the same adjacency graph as land territories but are never claimed, attacked as land, or fortified into. Their live state is `GameState.seas`: one entry per sea on the map, each listing every player currently holding **ships** there (`{ playerId, ships }[]`, omitting any player with `0`). A map with no seas has `seas: []` and none of what follows ever applies to a game played on it — there is no separate setting to turn ships off on a sea-capable map. Ships have no upkeep.

Seas are folded into `fogOfWar` exactly like land territories (see the `fogOfWar` paragraph above): a sea counts as "owned" by whichever player(s) currently hold ships there for the purpose of computing `visibleTerritoryIds`, so a recipient sees a sea (and everything else adjacent to it, land or sea) whenever they have ships there, in addition to every sea directly adjacent to one of their land territories. `GameState.seas` is filtered down to only the sea ids a recipient can currently see, the same way `territories` is; an unlisted sea is indistinguishable from one that simply doesn't exist, same as an unlisted land territory. `sailStartTerritoryId`/`sailEndTerritoryId` follow the same pair rule as `fortifyStartTerritoryId`/`fortifyEndTerritoryId` above (both kept as long as either endpoint is visible, both nulled together otherwise), and `sailPathTerritoryIds` (the sea-only chain between them, via `sailFullPath`) is redacted into runs the same way `fortifyPathTerritoryIds` is (see the `fogOfWar` paragraph above); `attackSeaTerritoryId`/`attackSeaDefenderId` are nulled together whenever `attackSeaTerritoryId` itself isn't visible to the recipient (a single-territory selection, unlike the two-territory land pairs).

Ships are bought during `'deploy'` (`game:buyShips` below), `3` troops each, from either a player's still-unspent `troopsToDeploy` pool for that turn or straight off the stack of a territory they own (leaving at least `1` troop behind there), and are placed into any sea directly adjacent to the paying territory. Buying ships never affects `troopsToDeploy`'s normal `'deploy'` end-of-phase rules (see `'deploy'` above) beyond decrementing the pool when paid that way.

A new phase, `'sail'`, sits between `'deploy'` and `'attack'` in the per-turn order (see the full phase list above) and lets a player move any number of their own ships, once, from one sea to a directly- or indirectly-connected sea (`game:sailSelectStart`/`game:sailSelectEnd`/`game:sail` below) — the same select-start/select-end/confirm shape as `'fortify'`, except the graph walked is sea-to-sea adjacency alone (ownership of the seas themselves is meaningless, there is none) and, unlike `'fortify'`, a sail may move every ship off the source sea, none need be left behind. Like `'attack'`/`'fortify'`/`'entrench'`/`'toxins'`, `'sail'` is skipped automatically the instant it would otherwise begin if the player has no ships anywhere with another sea to reach, re-checked the same way and at the same points as those other phases (see above).

During `'attack'`, a player with ships in a sea that also holds another player's ships may fight them (`game:attackSeaSelectStart`/`game:attackSeaSelectDefender`/`game:attackSea` below): the same dice-based combat as a land attack (attacker rolls up to `3` dice for however many ships it commits to this roll, defender rolls up to `defenceDice`, pairing largest to largest), with two differences — a tie favors neither side (no loss either way, rather than the defender winning), and the attacker may commit every ship it has there, none need stay uncommitted (even for `type: 'regular'`, unlike land's troops-minus-one cap). This is purely a fight over ships within one sea, never across two; there is no conquest or follow-up move for it, since a sea fight is never over territory. Ship fights do support the same `type: 'blitz'` option land attacks do (see `game:attackSea` below): committed ships fight to elimination of one side via the game's `blitz` setting exactly as a land blitz does, except every exchange along the way still treats a tie as neutral rather than favoring the defender. `'attack'` is only auto-skipped once neither a land attack nor a ship fight (nor a ready nuke) is available.

Ships also let a land attack **bridge** a sea: attacking from territory A to territory B that isn't directly adjacent to it is additionally allowed whenever some sea S is adjacent to both A and B, the attacker has at least one ship in S, and the attacker has more ships in S than B's current owner does. Only a single sea counts as a bridge; two seas in sequence never connect two land territories this way. A bridged attack otherwise behaves exactly like an ordinary adjacent land attack (same dice, same conquest and troop-move rules); the ships in S are unaffected by it either way. `attackFullPath`/`GameState.attackPathTerritoryIds`/`game:attacked`'s and `game:attackMoved`'s `path` all reflect the bridging sea (`[A, S, B]`) whenever an attack actually crosses one, or just `[A, B]` for an ordinary adjacent attack, the same way `fortifyFullPath` does below.

Ships also extend `'fortify'` (`fortification: 'Connected'` only) across water: the same chain search that walks through territories the mover owns also freely walks through any sea holding at least one of the mover's own ships, regardless of whose ships (if anyone else's) are also there, then back onto any owned land territory adjacent to that sea, and so on through as many further ship-held seas chained together as it takes to reach it. This is additive to land connectivity, never a replacement for it: a chain can freely mix land and sea hops, and two territories connected purely by land are completely unaffected. `fortifyFullPath`/`GameState.fortifyPathTerritoryIds`/`game:fortified`'s `path` reflect whichever sea hops a given move actually crosses, exactly as they already do for land-only hops. `fortification: 'Neighboring'` never extends across a sea this way (the two territories must share a land border directly, same as ever); `'Unrestricted'` never cared about adjacency of any kind in the first place.

For a player to be eliminated (see `eliminated` above), they must lose every territory, exactly as without ships — holding ships never keeps a player alive, and having none never eliminates one. The instant a player is recorded as eliminated (see the elimination paragraph under "Territory cards" below and `game:attack`/nukes/radiation elsewhere in this document), every ship they held in any sea is removed from the game.
...
`radiations` (`'off'` by default) marks some territories completely unusable: no owner, no troops, and inaccessible the same way a toxined territory is (can't be attacked, fortified into, selected, or counted toward anyone's `territoryCount`). `toxins` and `radiations` can both be on in the same game; `game:settings` no longer rejects either one for the other being on. A single territory is still never both toxined and radiated: `game:toxins` rejects toxining a territory the caller doesn't own, which a radiated territory never is, and `radiations`' `'dynamic'` rotation (see below) never moves onto a currently-toxined territory. `'expanding'` radiation is the one exception, since its growth can land on a currently-toxined territory like any other eligible neighbor, silently clearing that toxin the instant the territory turns radiated. Either way, toxining a territory and radiation's own "would split the map" check (see the `toxins` paragraph above) each treat the other's current territories as removed too, alongside their own. Because radiation is previewed a round before it lands, `game:toxins` additionally requires that the map stays connected against the previewed `radiationUpcomingTerritoryIds` (with the current toxins) as well as against the current `radiationTerritoryIds`. "Would split the map" means some non-removed land territory can no longer reach the others over land, portals, or sea (a sea territory is traversable, and a sea left cut off from everything else is fine). Sea territories can never be toxined or radiated: `game:toxins` rejects one with `invalid territory`, and radiation never picks one, neither at placement nor when moving or expanding. Capitals can never be radiated. Unlike toxins, radiation is never placed by a player action: the server decides it entirely on its own. Placement happens once, at `game:start`, *before* territories are dealt out (so `'Random'`/`'Semi'` dealing, and `'Custom'`'s claimable set, both exclude whatever's radiated from the start): `min(8, ceil(territoryCount / 10))` territories under `'static'`, `min(6, ceil(territoryCount / 12))` under `'dynamic'` (fewer, since they move around and would otherwise choke the map), or exactly `1` under `'expanding'` (`territoryCount` being the map's total). `radiationTerritoryIds` lists the currently-radiated territories; always empty under `'off'`.
...
`nukes` (`'off'` by default) lets players build and use nuclear weapons. While `'on'`, during their own `'deploy'` phase a player can spend `troopsToDeploy` on construction instead of placing those troops on the map: `game:buildNuke` starts a nuke (`5` troops, the first of `5` installments), `game:buildAntiNuke` starts an anti-nuke (`3` troops, the first of `5`), and `game:advanceNuke` pays one more installment on an existing in-progress project (see all three below). Construction is entirely manual: at most one installment per project per turn (gated on `roundNumber`, so a device can never finish in fewer than `5` of the owner's turns), and a project can be left unpaid for any number of turns in between. A finished project moves into that player's private `arsenal` and stays there until used. `nukeProjects` (in-progress projects) and `arsenal` are per-recipient: `game:state` only ever carries the recipient's own, never another player's, regardless of `fogOfWar`. The `'attack'` phase is no longer auto-skipped (see `turnPhase` above) while the player has any device in their `arsenal`. During their own `'attack'` phase a player can `game:launchNuke` at any territory that isn't theirs (enemy-owned, allied, unowned, toxined, and radiated targets are all allowed; a misfire onto an empty tile simply does nothing) — the rocket flies from the launcher's highest-troop capital, or their highest-troop territory if the game has no capitals, and the target is obliterated exactly as radiation wipes a territory (removed from `territoryOwners`, all troops and entrenchment gone), except `capitalTerritoryIds`, `territoryToxins`, and `radiationTerritoryIds` are left untouched, so a nuked capital tile is still a capital for whoever recaptures it. Losing a last territory this way eliminates the player (credited to the launcher's `playersKilled`, unlike radiation), with the same end-of-game check as any elimination. `game:deployAntiNuke` places an anti-nuke from the arsenal on one of the player's own territories, adding it to `antiNukeTerritoryIds`; it's bound to that territory forever (never listed in `game:state` for anyone but its current owner) and its ownership follows the territory's — capturing the territory hands the new owner the anti-nuke, and radiating or toxining the territory destroys it. When a nuke is launched, before it lands the server checks whether the target's owner has any anti-nuke on a territory connected (through that player's own territories, portal edges included when active) to the target; if so, that one anti-nuke is consumed and the nuke is voided (no obliteration), automatically, with no choice for the defender. Every launch — landed or intercepted — broadcasts `game:nukeLaunched` (see below) to everyone regardless of `fogOfWar` and records a `'nuke'`-type `ReplayFrame` (see "Shared types" above); the obliteration itself still rides the normal fog-filtered `game:state`, so a fogged recipient sees and hears the blast without learning who or how many troops were there.
...
No further `game:*` actions are accepted once `state` is `'ended'` (they all require `'playing'`): the game just sits, still joinable as a spectator (see `game:join` below), until everyone still viewing it (players and spectators alike, going by `connected`) has closed its screen, at which point the server deletes it (and it stops appearing in `GET /games/live`), subject to the same 5-second reconnect grace period described above, so a refresh doesn't delete an ended game either.
...
- **Query params** (all optional): `page` (1-based, default 1), `pageSize` (default 20, max 100), `playerIds` (repeated — one or more account ids, from `GET /games/players/search`; a game must currently seat **every** listed id, capped at 10; identity is resolved server-side from the in-memory player↔account map and never included in the response), `name` (case-insensitive substring match on the game's `name`), `mode` (an exact game-mode name), `mapName` (exact — matched against the summary's `mapName`), `generatedMap` (`1` — any game whose map is generated), `mapGenerationSize` / `mapGenerationType` / `mapGenerationFill` (exact match against `mapGeneration.size` / `mapGeneration.type` / `mapGeneration.fill`; the UI presents map / generated as one single-select and only shows the generated params once "Generated" is picked, so a request never mixes `mapName` with the generated params in practice), `playersMin` / `playersMax` (inclusive range on `playerCount`), `minRounds` / `maxRounds` (inclusive range on `roundNumber`), any **game-setting** key (`blitz`, `defenceDice`, `cards`, `placement`, `fortification`, `entrenchments`, `toxins`, `portals`, `radiations`, `nukes`, `starvation`, `roundTroops`, `bounties`, `supplyLines`, `fogOfWar`, `alliances`, `turnDuration`, `disconnectBot*`) taking one of that setting's own values (exact match against the game's setting value; the settings themselves are not returned in the response), `phase` (`lobby` / `playing` / `ended` — exact match on `state`), `hasPassword` (`1` / `0`), `hasBots` (`1` / `0` — exact match on the summary's `hasBots`), `sort` (`newest`, `players`, `rounds`, or `name`; `newest` orders by `createdAt`) with `sortDir` (`asc` / `desc`, default `desc`) — the home UI exposes these as eight one-shot options (Newest / Oldest / Most players / Fewest players / Most rounds / Fewest rounds / Name A–Z / Name Z–A). Filtering, sorting, and pagination all run in memory over the live game list — this is not a database query. Same filter vocabulary as `GET /games/history`, minus the history-only fields (dates, duration, `outcome`, `position*`, `mine`) that don't apply to games still in progress. `playerIds` is the one identity-touching param, but it reads the in-memory map rather than the session, so the route stays cookie-free.
- **Response:** `{ games: GameSummary[]; total: number; page: number; pageSize: number }` — the public-games list (private games omitted, `hasPassword` filled in) after the query above. `total` is the match count across all pages. Fetching this also reconciles server-side game metadata for games that have since been destroyed. This route is public: it runs ahead of the identity middleware, reads no cookie, and never slides a session or mints one, so the ~5s poll costs no session-store work.

### `GET /games/history`
- **When sent:** the "Games" page (`/games/replay`) loads and on every filter / page change. One request per rendered table page — the replay and its map are separate requests (below).
- **Query params** (all optional): `page` (1-based, default 1), `pageSize` (default 20, max 100), `playerIds` (repeated — one or more account ids, from `GET /games/players/search` below; a game must include **every** listed id, via Mongo `$all`, capped at 10 ids to match the UI's selection limit), `playersMin` / `playersMax` (inclusive range on the game's player count, done via `$expr` on `$size` of `players` so it needs no denormalised field), `name` (case-insensitive substring match on the game's `name`, via a Mongo `$regex` with the input regex-escaped), `mode` (an exact game-mode name), `mapName` (exact map name; resolved to that name's `mapId`(s) in the `replay_maps` collection before filtering `games`), `startedFrom` / `startedTo` and `endedFrom` / `endedTo` (each an inclusive range, epoch milliseconds, matched directly against the stored `startedAt` / `endedAt` fields — the UI converts its `datetime-local` inputs to milliseconds before sending), `durationMin` / `durationMax` (inclusive range in **minutes** on `endedAt - startedAt`, evaluated via `$expr` since it's not a stored field), `generatedMap` (`1` — matches any game with a non-null `mapGeneration`, i.e. any generated map regardless of the other params), `mapGenerationSize` / `mapGenerationType` / `mapGenerationFill` (exact match against `mapGeneration.size` / `mapGeneration.type` / `mapGeneration.fill`, further narrowing `generatedMap`; a player-map game's `mapGeneration` is always `null`, so these can never match one). The UI presents `mapName` and `generatedMap` as one single-select ("Any" / "Generated"), and only shows the type/fill/size dropdowns once "Generated" is picked, so a request never mixes `mapName` with `generatedMap`/`mapGenerationSize`/`mapGenerationType`/`mapGenerationFill` in practice — though nothing stops a raw request from passing an explicit `mapName` (e.g. a player map's name) or combining them, which just yields an always-empty match. Any **game-setting** key (`blitz`, `defenceDice`, `cards`, `placement`, `fortification`, `entrenchments`, `toxins`, `portals`, `radiations`, `nukes`, `starvation`, `roundTroops`, `bounties`, `supplyLines`, `fogOfWar`, `alliances`, `turnDuration`, `disconnectBot*`) taking one of that setting's own values (each an exact `settings.<key>` match), `minRounds` / `maxRounds` (inclusive range on rounds played), `hasBots` (`1` / `0` — `1` matches games with at least one `players[].isBot`, `0` matches games with none), `outcome` (`won` / `lost`), `positionMin` / `positionMax` (1-based inclusive range over the caller's finishing position — "position" throughout, since "placement" is the troop-setup game setting), `sort` (`newest`, `rounds`, or `position`) with `sortDir` (`asc` / `desc`, default `desc`) — the "Games" UI exposes these as six one-shot options (Newest / Oldest / Most rounds / Fewest rounds / Best position / Worst position), `mine` (`1` to restrict to the caller's own account, folded into the same `players.userId` match as `playerIds` rather than a separate clause), `rankUserId` (an account id whose own `results`-derived rank/won fills `yourRank` for each row instead of the session's — used by the player profile page, see `GET /players/:username` below, to show that player's own placement in their games regardless of who's viewing; takes priority over the session when both are present). Only `mine`, `outcome`, `position*`, and `sort=position` consult the session (via the `anx` cookie); everything else, `rankUserId` included, is identity-free. The caller-relative fields (finishing position, whether they won) are computed in the aggregation from the game's `results` / `winnerIds` matched to the relevant `players[].playerId` (the session's, or `rankUserId`'s when given), so they work on every stored game regardless of whether the newer denormalised `players[].rank`/`won` are present.
- **Response:** `{ games: Row[]; total: number; page: number; pageSize: number }` where `Row` is `{ id, name, startedAt, endedAt, mapName, gameMode, roundNumber, playerCount, winnerIds, winnerNames, yourRank, settings, players: { name, isBot, color, team }[] }`. `yourRank` is filled from the session when logged in (regardless of `mine`), or from `rankUserId` when given (session takes a back seat in that case), else `null`. `id` opens the replay at `/games/replay/<id>`. `startedAt` / `endedAt` are ISO timestamps converted from the stored millisecond fields. `mapName` and every player's `name` are resolved from `replay_maps`/`users` for just this page's rows (see the `games` collection below) — not stored on the documents themselves.
- **No access restriction:** any caller may page and filter across **every** stored game, not just their own — the UI simply defaults `mine` on for a logged-in user. `mine=1` from an anonymous caller returns an empty page. A game is recorded on end only if at least one full round completed (`roundNumber >= 1`, same gate as ELO and the `replay_maps` archive); an **offline** game is never sent to the server or persisted anywhere, so it never appears here and has no replay to return to.

### `GET /games/players/search`
- **When sent:** as the user types into the "Games" page's Players filter (debounced client-side), once per keystroke pause.
- **Query params:** `q` (required; the typed text, case-insensitive prefix match against `username`, capped at 100 characters; index-backed via the stored lowercased username). An empty or missing `q` returns `[]` without querying.
- **Response:** up to 8 `{ id, username }` matches, alphabetical. Only registered accounts are searchable this way — a bot or a guest who never logged in has no `users` document and can't be filtered on by identity, only by browsing. Public and unrestricted, like the rest of this router.

### `GET /games/replay/:id`
- **When sent:** the standalone replay page (`/games/replay/:id`) loads, once, after (and separately from) the list request that linked to it. The replay page takes **all** its data from here; it never reuses in-memory state from a game just played.
- **Response:** the full stored game document (see the `games` collection below), or `404 { ok: false, error: 'not found' }`. Public and unrestricted — any id, any caller; like `GET /games/live` it runs ahead of the identity middleware, reads no cookie, and never mints or slides a session. (An offline game is never requested here — it has no id and no stored document.)

### `GET /players`
- **When sent:** the "Players" page (`/players`) loads and on every filter / sort / page change.
- **Query params** (all optional): `page` (1-based, default 1), `pageSize` (default 20, max 100), `username` (case-insensitive substring match, capped at 10 characters to match the username length limit), `eloMin` / `eloMax` (inclusive range), `gamesMin` / `gamesMax` (inclusive range on total games played), `sort` (`elo` (default), `username`, or `games`) with `sortDir` (`asc` / `desc`, default `desc`).
- **Response:** `{ players: Row[]; total: number; page: number; pageSize: number }` where `Row` is `{ id, username, elo, gamesPlayed }`. `id` opens that player's profile at `/players/<username>`. Public and unrestricted — every registered account is listed, not just those with games played.
...
### `GET /players/:username/elo-history`
- **When sent:** the player profile page loads, to draw the "Elo over time" chart.
- **Response:** `{ startedAt, endedAt, elo, eloDelta }[]`, oldest first, one entry per stored game the account played (`elo` = elo before the game, `eloDelta` = change applied by it, so the elo after is `elo + eloDelta`); `[]` for an unknown username or an account with no games. Public and unrestricted.

### `GET /replay-maps/:id`
- **When sent:** the replay page, as its own request after `GET /games/replay/:id` returns, using that document's `mapId`.
- **Response:** `{ name, territories, seaTerritories, bonuses, wraps, image, imageMime }` (`image` base64) — the archived map geometry and picture from the `replay_maps` collection, so the replay page can render the board without the geometry ever being embedded in the game document. `404 { ok: false, error: 'not found' }` if unknown. Public and unrestricted — any id, any caller; runs ahead of the identity middleware and touches no cookie.

### Map library (`/player-maps`)

A **player map** is a user-authored map (territory graph + continent bonuses + a background image) stored in the `player_maps` collection with an `authorId` (a registered account), a `likeCount`, a `dangerous` flag, and an optional `generation` blob (`{ seed, size, type, fill, seas }`) recording the map-generation settings used in the editor when the map originated from a generated map (`null` for hand-drawn or imported maps; cleared when a raster image is imported over it). Anyone may browse and use player maps; only the author (logged in) may edit or delete one, and any logged-in user may like or report one. **Five distinct reports** mark a map `dangerous` (same rule as profile pictures): it then vanishes from every browse and is unplayable, except its author still sees it in their own list and may delete it (never edit it). Every saved image passes the same OpenAI moderation as profile pictures; a flagged image is rejected. These routes sit after the identity middleware (`/player-maps` bodies are parsed with an 8 MB limit); all mutations are `POST` (CORS allows no `PUT`/`DELETE`).

A player map may also carry `seaTerritories: { id, x, y, neighbors }[]` — nodes drawn in the editor as large blue hexagons. They sit in the same adjacency graph as `territories` (their `neighbors` may reference land or sea ids and vice versa, and the whole graph including seas must stay connected), but have no `continentId` and are excluded from continent membership, continent-size limits, and `territoryCount`. Seas are neutral, gameplay-active territories that can hold ships — see "Seas and ships" below — and are included alongside `territories`/`bonuses` in `game:mapGenerated`/`game:selectPlayerMap`.

A player map also carries `wraps: { a, b, x, y }[]` (stored required in `player_maps` and `replay_maps`, empty by default). Edges are normally drawn across the map border only when their endpoints are more than 80% of the map width (or height) apart; a `wraps` entry forces the edge between territories `a` and `b` to wrap horizontally (`x`) and/or vertically (`y`) regardless of distance. It only affects how the edge and arrows along it are drawn, never adjacency or gameplay.

- **`GET /player-maps`**: paginated browse. Query (all optional): `page` (default 1), `pageSize` (default 24, max 60), `q` (case-insensitive: substring match on map name, or prefix match on author username), `author` (a 24-hex account id, restricts to that author's maps, used by the profile page), `mine` / `notMine` / `liked` / `notLiked` (`1`; each is silently ignored without a session, so an anonymous caller cannot use any of the four), `territoryMin` / `territoryMax` (inclusive range on territory count), `genType` (`terrain` / `dungeon` / `temple`), `genFill` (`full` / `mixed` / `sparse`), `genSize` (`small` / `medium` / `large` / `xlarge`) (these three match on the stored `generation` blob, so they only ever return generated maps), `sort` (`mostLiked` (default), `newest`, `oldest`, `nameAsc`, `nameDesc`). Response `{ maps: Row[]; total; page; pageSize }` where `Row` is `{ id, name, authorId, authorName, territoryCount, continentCount, likeCount, liked, mine, dangerous, generation, createdAt }` (`liked` / `mine` resolved against the session, `false` when anonymous; `generation` is `{ seed, size, type, fill, seas }` or `null`). `dangerous` maps are excluded unless `mine` and the caller is the author.
- **`GET /player-maps/:id`**: the full map for the editor or for play, `{ id, name, authorId, territories, seaTerritories, bonuses, wraps, image (data URL), imageMime, generation, dangerous, likeCount, mine, createdAt }`. `404` if unknown, or if `dangerous` and the caller is not the author.
- **`GET /player-maps/:id/image`**: the raw image bytes (for `<img>` thumbnails). `404` if unknown, or if `dangerous` and the caller is not the author.
- **`GET /player-maps/mine/names`**: the names of the caller's own maps (logged in), so the editor can warn about a name clash before saving. `{ ok: true, names: string[] } | { ok: false, error: 'not logged in' | 'server error' }`.
- **`POST /player-maps`**: create (logged in). Body `{ name (1–40 chars, unique per author), territories, seaTerritories, bonuses, wraps, image (data URL, ≤6 MB, png/jpeg/webp), generation }` where `generation` is `{ seed (1–20 chars), size, type, fill, seas }` or `null`/omitted. Geometry is validated server-side (ids unique across `territories` and `seaTerritories` together, neighbours valid and mutual, the whole graph — land and sea — connected, at most 1200 territories total (`MAX_CONTINENTS` x `MAX_CONTINENT_SIZE`, land + sea combined), 0–30 continents each sized 2–40 land territories (a land territory may have `continentId` `-1` for no continent), bonuses 2–25, `wraps` a list of `{ a, b, x, y }` naming existing edges, each at most once, with at least one of `x`/`y` true), the `generation` enums are checked, then the image is moderated. Response `{ ok: true, id } | { ok: false, error }` where `error` is one of `'not logged in'`, `'too many requests'`, `'invalid name'`, `'name taken'` (another of the caller's maps already has that name), `'invalid territories'`, `'invalid seas'`, `'invalid bonuses'`, `'invalid wraps'`, `'not enough territories'`, `'too many territories'`, `'duplicate territory id'`, `'invalid continent'`, `'invalid neighbor'`, `'asymmetric neighbor'`, `'territories not all connected'`, `'invalid continent size'`, `'invalid generation'`, `'invalid image'`, `'image rejected'`, `'moderation failed'`, `'server error'`.
- **`POST /player-maps/:id`**: replace an existing map's geometry, image, name and `generation` (logged in, author, not `dangerous`). Re-runs validation and moderation, and **keeps** the existing likes and reports. `{ ok: true } | { ok: false, error }` where `error` is `'map not found'` (unknown id, or the caller is not the author), `'map unavailable'` (the map is `dangerous`), `'name taken'` (another of the caller's maps already has the new name), or any of the `POST /player-maps` validation / moderation errors above.
- **`POST /player-maps/:id/delete`**: delete own map (logged in, author); also clears its likes and reports.
- **`POST /player-maps/:id/like`** / **`POST /player-maps/:id/unlike`**: toggle the caller's like (logged in). Idempotent.
- **`GET /player-maps/:id/liked`**: `{ ok: true, liked: boolean }`, whether the caller (logged in) likes that map; `{ ok: false, error }` when anonymous or the map no longer exists. Used by the lobby, end-of-game and replay screens to offer a like toggle on the game's player map.
- **`POST /player-maps/:id/report`**: record a distinct report from the caller (logged in). At five distinct reporters the map is marked `dangerous`. `{ ok: false, error }` where `error` is `'map not found'` (unknown id, or already `dangerous`), `'cannot report yourself'` (the caller is the map's author), or `'already reported'` on a repeat.

### `games` collection (server-side, not a wire type)
A strict-schema MongoDB collection, one document per finished game (created on game end when `roundNumber >= 1`). Everything that does **not** change over the game sits at the top level — `name`, `playerMapId` (the `player_maps` id the game was played on, `null` for a generated map; optional, absent on games stored before it existed; the map may since have been edited or deleted, the replay itself always renders the `mapId` snapshot), `mapId` (→ `replay_maps` collection; the map's own document holds `name`, so it is looked up rather than duplicated here; a game played on a player map is snapshotted there by content hash too, so its replay survives the player map being edited or deleted), `mapGeneration`, `originalHostId`, `startedAt` / `endedAt` (epoch milliseconds — set by the engine itself: `startedAt` when the lobby transitions to `'playing'`, `endedAt` at the moment the win condition is checked and the game moves to `'ended'`, both well before the async map-persist-then-insert chain that follows; `exportGame` refuses to export — and so nothing gets stored — unless both are set, which is guaranteed for any game that reached `roundNumber >= 1` the normal way), `winnerIds`, `roundNumber`, `playerCount` (denormalised `players.length`, indexed for the filter), `capitalTerritoryIds` (the static set placed during the `'capital'` phase, so a replay can draw capital markers), `settings` (every rule field), `players` (`{ playerId, userId, name, isBot, botDifficulty, botPersonality, team, color, turnOrder, rank, won, elo, eloDelta }`, `turnOrder` = seat order once the game started, `rank` = 0-based finishing position, `won` = in `winnerIds`, `elo` = the account's elo when the game started (integer >= 0) and `eloDelta` = the change this game applied to it (integer, negative for a loss); both are required and `0` / `0` is an ordinary value; a bot or anonymous player holds `0` / `0` since it has no elo, and an account in a game where fewer than two accounts played holds its current elo with a delta of `0`; `name` is `null` whenever `userId` is set — a registered account's current username is looked up from the `users` collection instead, so a renamed account reads with its new name on old games too — and only populated for a bot or a guest who never logged in, since neither has a `users` document to resolve from), `results` (the end-of-game table, ranked: `rank`, `team`, `eliminated`, `surrendered`, `playersKilled`, and each cumulative stat), and `serverLog` (`{ afterFrame, type, payload }[]` — the same event types `game:logs` replays, but recorded from an all-seeing viewer so fog of war never redacts them; the `game:allianceFormed` / `game:allianceTerminated` entries also carry a `playerId` — the acting player — alongside `withId`, since the all-seeing view has no "recipient" perspective to imply it. `afterFrame` is the `replay.frames` index the entry became true at, so the replay's log panel can reveal lines in step with the map; games stored before this field existed omit it, and a client then shows the whole log from the start). Everything that **does** evolve sits in `replay`: `initialTerritories`, `initialRadiation`, and `frames` — one ordered stream of `{ kind: 'action' | 'turn' | 'chat' | 'emoji', ... }`. An `'action'` frame carries the same evolving state as a `ReplayFrame` (see above) — `hands`, `playerStates` and the rest — except `territories` is replaced by `mapDelta` (only the territories that changed since the previous frame; the client folds these back into full snapshots) and `seaShips` is replaced by `seaShipsDelta` (only the `{ seaTerritoryId, playerId, ships }` entries that changed since the previous frame, `ships: 0` meaning that player's ships there were removed; folded back into full per-sea snapshots the same way); `'turn'`, `'chat'`, and `'emoji'` frames carry a turn-started marker, a chat message, or a sent emoji, spliced into the stream at the point they occurred. `mapName` and every player's display `name` are filled back in — from `replay_maps`/`users` — on the way out to `GET /games/history` and `GET /games/replay/:id`, so the wire carries resolved names, never the stored `null`s.

### `friendships` collection (server-side, not a wire type)
A strict-schema MongoDB collection, one document per relationship (request or friendship) between two accounts: `{ userA, userB, requesterId, status: 'pending' | 'accepted', createdAt }`. `userA` / `userB` are the two account ids sorted ascending — a canonical pair key, so A→B and B→A map to the same document — with a unique compound index on `{ userA, userB }` (backed by `{ userB }` for the reverse lookup). `requesterId` (always `userA` or `userB`) is who sent the request; it distinguishes an incoming from an outgoing pending request, and a `pending` doc can only be accepted by the account that is *not* the requester. Accepting flips `status` to `'accepted'`; rejecting, cancelling, and unfriending all delete the document. No blocked/rejected state is kept — a removed relationship can simply be requested again. Usernames and elo for `GET /friends` are resolved from the `users` collection per request, never duplicated here.

### `messages` collection (server-side, not a wire type)
A strict-schema MongoDB collection, one document per direct message: `{ senderId, recipientId, text, createdAt }` — `text` 1–1000 chars, `createdAt` a `Date`. Indexed on `{ senderId, createdAt }` and `{ recipientId, createdAt }`, so "everything a player sent" and "everything a player received" are each a single index scan. There is no stored conversation key: `GET /messages` fetches every message where the caller is sender *or* recipient and groups them by the other account in memory. Rate limits are enforced per send straight from these indexes — no in-memory counters: a send is rejected `too fast` if the caller's most recent message is under 1s old, and `daily limit reached` once they have 100 messages with `createdAt` within the last 24h. Usernames and elo are resolved from the `users` collection per request.

### `blocks` collection (server-side, not a wire type)
...
- **Body:** `{ email, password, stayLoggedIn?: boolean }` — `stayLoggedIn` defaults true; when false the rotated `anx` cookie is a session cookie (no `Max-Age`) rather than the multi-year persistent one.
- **Purpose:** verify the password (normalised email match), mint a fresh session token, store it hashed in `sessions`, and return it in `Set-Cookie`; the pre-login token is discarded. No engine work: `GET /session` re-reads the rotated cookie, and the next time the client opens a game socket its `player:identify` adopts the account's in-memory player (resuming that account's game if it has one). An unconfirmed account is refused and re-sent a fresh confirmation email. One password verification always runs even for an unknown email (timing).
- **Response:** `{ ok: true, username, clientSettings, gameSettings } | { ok: false, error }`.
- **Errors:** `in a game` (the caller has a game socket open, or is seated in a `playing` game), `invalid credentials`, `email not confirmed`, `too many requests` (per-IP, or 8 wrong-password attempts on this email — normalised, so Gmail-style aliases share one count — within 15 min; the correct password always succeeds and clears that count), `server error`.
...
- **Purpose:** record one report against that account's current picture. A user may report a given picture at most once. Once five different users have reported the same picture, it is immediately marked dangerous and stops being served (`GET /players/:username` and `GET /account` return `picture: null`) until that account uploads a replacement.
...
- **Response:** `{ friends: Person[], incoming: Person[], outgoing: Person[] }` where `Person` is `{ id, username, elo }` (the *other* account in each relationship). `friends` are accepted friendships (bidirectional); `incoming` are pending requests this account must accept or reject; `outgoing` are pending requests it has sent. All three newest-first. A profile page reads this same payload to show its friendship state (none / friends / request sent / request received) toward the account being viewed. An anonymous caller gets `{ friends: [], incoming: [], outgoing: [] }`.

### `POST /friends/requests`
- **When sent:** "Add friend" on the Friends search results or a player's profile (`/players/<username>`).
- **Body:** `{ userId }` — the target account's id (from `GET /friends`, `GET /games/players/search`, or `GET /players/:username`).
- **Purpose:** create a `pending` friendship with this account as requester. If the target has already sent *this* account a pending request, that request is accepted instead (the friendship becomes `accepted`).
- **Errors:** `not logged in`, `cannot friend yourself`, `user not found`, `already friends`, `request already sent`, `server error`.

### `POST /friends/accept`
- **When sent:** "Accept" on an incoming request.
- **Body:** `{ userId }` — the account that sent the request.
- **Purpose:** mark the pending friendship `accepted`. Both accounts are now friends of each other.
- **Errors:** `not logged in`, `no request` (no pending request from that account), `server error`.

### `POST /friends/remove`
- **When sent:** "Reject" an incoming request, "Cancel" an outgoing one, or "Remove" an existing friend.
- **Body:** `{ userId }` — the other account.
- **Purpose:** delete the friendship document for the pair, whatever its state. Idempotent — `{ ok: true }` even if nothing matched.
- **Errors:** `not logged in`, `server error`.

### `GET /messages`
- **When sent:** while a logged-in client is on any non-game screen — the burger menu polls it on a fixed short interval (~15s, visible tab only) to keep its unread badge current, the "Messages" page (`/messages`) shares that same poll, and it is re-fetched after every message/block mutation below. Logged-in only — the client hides the menu entry and redirects `/messages` to home for an anonymous visitor. A player's profile (`/players/<username>`) shows a "Send message" button below "Add friend" (logged-in viewer, not self) that just navigates to `/messages?to=<username>`, which pre-opens that conversation. Unread counts are tracked purely client-side (a per-conversation last-seen timestamp in `localStorage`); the server keeps no read state.
- **Response:** `{ conversations: Conversation[], blocked: BlockedPlayer[] }`. `Conversation` is `{ userId, username, elo, messages: { fromMe, text, createdAt }[] }` — one per other account the caller has exchanged messages with *and has not blocked*, `messages` oldest-first, conversations newest-activity-first (by the timestamp of the most recent message either way). `BlockedPlayer` is `{ userId, username, elo }` and lists **every** account the caller has blocked. A blocked account never appears in `conversations` (only in `blocked`); its conversation reappears intact once unblocked. An anonymous caller gets `{ conversations: [], blocked: [] }`.

### `POST /messages`
- **When sent:** "Send" in a conversation.
- **Body:** `{ userId, text }` — the recipient account's id and the message body (≤1000 chars, trimmed).
- **Purpose:** store one message from the caller to `userId`.
- **Errors:** `not logged in`, `cannot message yourself`, `user not found`, `blocked` (the caller has blocked the recipient), `empty message`, `message too long`, `message contains profanity` (same `obscenity` check as in-game chat — the client blocks it first, the server rejects it authoritatively), `too fast` (under 1s since the caller's last message), `daily limit reached` (100 in the last 24h), `server error`.

### `POST /messages/block`
- **When sent:** "Block" in a conversation.
- **Body:** `{ userId }` — the account to block.
- **Purpose:** create the block (`blocks` collection). Idempotent. Asymmetrical — see the `blocks` collection above.
- **Errors:** `not logged in`, `cannot block yourself`, `user not found`, `server error`.

### `POST /messages/unblock`
- **When sent:** "Unblock" in a conversation or the blocked-players list.
- **Body:** `{ userId }` — the account to unblock.
- **Purpose:** delete the block document. Idempotent — `{ ok: true }` even if nothing matched.
- **Errors:** `not logged in`, `server error`.

## Client → Server

### `player:identify`
- **When sent:** once, immediately after the socket connects. The client only ever opens a socket while it is on a live game's screen, so this is sent on entering a game, on reload/reconnect while in a game, and when navigating directly between game URLs. Leaving the game screen (to `home` or any other page) closes the socket entirely rather than re-identifying as `'home'`. The client always waits for `GET /session` to return before opening the socket, so the handshake carries the `anx` cookie and the identity is resolved from one token — never a throwaway one minted by the handshake racing an in-flight `GET /session`.
...
### `game:create`
- **When sent:** on the game screen, right after `player:identify`, when the URL names a game that doesn't exist yet (see `game:join`). "Create Online" on `home` picks a free name (from the last `GET /games/live`) and navigates to its URL; the game screen then creates it via this call.
- **Purpose:** create a game with default settings: name `Game with <player name>` unless `name` is given, **no map** (the host generates one or picks from the library before `game:start`, see `game:generateMap` / `game:selectPlayerMap`), 2 slots, `Supremacy` game mode, `Balanced` blitz, 2 defence dice, `Constant` cards, `Random` placement, entrenchments off, toxins off, portals off, radiations off, starvation off, round troops off, bounties off, supply lines off, fog of war off, alliances off, random disconnect bot difficulty and personality (see "Bots" below), 120s turn duration; make the caller host and move their socket into the room.
- **Content:**
  ```ts
...
- **Content:** (all fields optional, only send what changed)
  ...
  Raising `slots` immediately promotes connected spectators, in queue order, into the newly-open slots (each gets a team of 0, a random `color`, and a place in the host-priority order), until the slots are full or no connected spectator is left; disconnected spectators are skipped and stay queued.
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `not the host`, `game already started`, `invalid alliances`, `invalid banned players`, `invalid blitz`, `invalid bounties`, `invalid cards`, `invalid defence dice`, `invalid disconnect bot difficulty`, `invalid disconnect bot personality`, `invalid entrenchments`, `invalid fog of war`, `invalid fortification`, `invalid game mode`, `invalid name`, `invalid nukes`, `invalid password`, `invalid placement`, `invalid team`, `invalid portals`, `invalid radiations`, `invalid slots`, `invalid starvation`, `invalid supply lines`, `invalid toxins`, `invalid turn duration`, `invalid round troops`, `invalid visibility`, `game name already in use`.

### `game:generateMap`
- **When sent:** the host of a `lobby` game requests a procedurally generated map, from the "Generate" entry in the map picker. Sending it again (any number of times, with the same or different `seed`/`size`/`type`/`fill`/`seas`) replaces whatever map the game currently has, exactly like `game:selectPlayerMap` would. (A logged-in user can also generate maps in the in-app editor and save them to the library: that path is a plain image upload and goes through the same OpenAI moderation as any player map; a map generated on the fly here is server-rendered, so it is not moderated.)
- **Purpose:** build a brand-new map server-side from a random seed and a small set of parameters (a connected territory graph, continents, and a rendered image), and make it the game's active map (`GameState.mapName` becomes the generated map's `<Type> (<Fill>-<Size>)` label, e.g. "Terrain (Sparse-Large)" - display-ready but not unique, since two seeds with the same type/fill/size/seas share it; `GameState.mapGeneration` is set to the `{ seed, size, type, fill, seas }` used, and `GameState.playerMapId` goes to `null`). `type` selects the generation style: `terrain` is the Perlin-island Voronoi map with oceans and lakes; `dungeon` and `temple` are room-and-corridor maze layouts (a rectangular grid maze and a concentric-ring maze respectively) with loops and inaccessible regions, and for those two the rendered image is cropped tight around the playable area (unless `fill` is `full`), so a generated maze map's image is not necessarily 16:9. When `seas` is true, the inaccessible space is instead carved into a handful of large sea regions (rendered distinctly, bordered like territories) and the map's landmasses/maze rooms connect to each other only through those seas rather than through invisible crossings; these seas are gameplay-active (see the Map library section for the equivalent `seaTerritories` concept used by player maps, and "Seas and ships" below) and are included in `game:mapGenerated`'s `seaTerritories`. The same `seed` with the same `size`/`type`/`fill`/`seas` always produces the exact same map; the actual resulting territory/continent counts are influenced by the random seed and aren't guaranteed to hit any particular number, only to trend toward the requested `size`'s range and typical continent sizing (continents are usually 2–15 territories, 7 most common). The caller must be host and the game must still be `lobby`. On success the server broadcasts `game:mapGenerated` (see below) to everyone currently in the game's room, then sends an Ack to the caller and `game:state` to everyone else.
- **Content:**
  ```ts
  {
    ...
    type: 'terrain' | 'dungeon' | 'temple';           // generation style
    fill: 'full' | 'mixed' | 'sparse';               // playable-area share; sparse leaves the most sea / inaccessible space
    seas: boolean;   // carve the inaccessible space into large, bordered seas instead of invisible crossings
  }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `not the host`, `game already started`, `invalid seed`, `invalid size`, `invalid type`, `invalid fill`, `invalid seas`, `server error`.

### `game:selectPlayerMap`
- **When sent:** the host of a `lobby` game picks a map from the library ("Choose from library" in the map picker). Sending it replaces whatever map the game currently has (generated or another player map), exactly like `game:generateMap`.
- **Content:** `{ mapId: string }`, a `player_maps` document id (see the Map library section under "HTTP API").
- **Purpose:** the server loads that player map from the database (rejecting a `dangerous` one), then sets it as the game's active map: `GameState.mapName` becomes the map's name and `GameState.playerMapId` its id, `GameState.mapGeneration` goes to `null`. It then behaves exactly like `game:generateMap`: it broadcasts `game:mapGenerated` (see below) to everyone in the room with the geometry and image, then an Ack to the caller and `game:state` to everyone else. Caller must be host and the game must still be `lobby`. Offline games handle this the same way, fetching the map over `GET /player-maps/:id`.
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `not the host`, `game already started`, `invalid map`, `map unavailable`, `server error`.

### `game:mapGenerated`
- **When sent:** by the server, never by the client. Broadcast to every socket in a game's room immediately after a successful `game:generateMap` **or `game:selectPlayerMap`** (before that call's Ack/`game:state`, so it's always available by the time `GameState.mapName` changes for any recipient); also sent, targeted at just that one socket, to a player or spectator who `game:join`s or reconnects into a game whose active map is currently a generated **or player** map, since `GameState` never carries map geometry or images. (A client can also re-fetch a player map itself via `GET /player-maps/:id` using `GameState.playerMapId`.)
- **Purpose:** deliver the full generated or player map (`territories` (a connected graph), `bonuses` (per continent), and a ready-to-use image), since `GameState` carries none of that and there are no map files on disk.
- **Content:**
  ```ts
  {
    name: string;        // equal to GameState.mapName from this point on: for a generated map, the `<Type> (<Fill>-<Size>)` label, e.g. "Terrain (Sparse-Large)"; for a player map, an arbitrary user-chosen name
    territories: { id: number; continentId: number; x: number; y: number; neighbors: number[] }[];  // continentId -1 = belongs to no continent
    seaTerritories: { id: number; x: number; y: number; neighbors: number[] }[];
    bonuses: number[];   // indexed by continentId; may be empty
    wraps: { a: number; b: number; x: boolean; y: boolean }[];   // edges (a, b) forced to wrap around the map horizontally (x) and/or vertically (y) regardless of distance; empty for generated maps
...
- **Purpose:** seat a new bot (see "Bots" above) in an open slot, with a random `team`/`color` exactly as a joining human would get. The caller must be host, the game must still be `lobby`, and it must have an open slot (`playerIds.length < slots`). `difficulty`/`personality` may each be `'random'`, resolved once, immediately, to a concrete value (independently of each other), but that value stays hidden: `GameState.players` keeps reporting `'random'` for it until the game ends (see "Bots" above); the bot's `name` is derived from the profile (e.g. `Killer (Hard)`, or `Random (Hard)` with a hidden personality), not unique, and changes if the profile is later changed via `game:setBotProfile`.
- **Content:**
  ```ts
  {
...
### `game:placeTroop`
- **When sent:** in `'Semi'` or `'Custom'` games only, the player whose turn it currently is to place, during `'troop'`, deposits some of their turn's troops (see `turnPhase` above) on one of their own territories, same as `game:deploy` (including the right-click shortcut).
...
### `game:deploy`
- **When sent:** the player whose turn it currently is, during `'deploy'`, places some of their troop pool (see `turnPhase` above) on one of their own territories, or, by right-clicking a territory, as much of the pool as it can take.
- **Purpose:** add `troops` to `territoryId`'s troop count and deduct them from the caller's remaining pool for the turn, clearing `selectedTerritoryId` in the process. Once the pool reaches `0`, the server auto-advances `turnPhase` to `'attack'` (same as a `game:nextPhase` call would) unless the caller can currently play a card set, in which case `turnPhase` stays `'deploy'` (see "Territory cards" above). The caller must be the player at `turnPlayerIndex`, the game must be `playing` and in `'deploy'`, `territoryId` must be owned by the caller, `troops` must be an integer from `1` up to the caller's remaining pool, while `supplyLines` is `'on'`, `territoryId` must be reachable from one of the caller's supply hubs (see the `supplyLines` paragraph above), and the deposit must not take `territoryId` above `1000000` troops.
- **Content:**
  ```ts
  { territoryId: number; troops: number }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not deploy phase`, `territory not owned`, `invalid troops`, `territory troop cap exceeded`, `territory not connected to supply hub`.

### `game:buyShips`
- **When sent:** the player whose turn it currently is, during `'deploy'`, buys one or more ships from the buy-ships control (see "Seas and ships" above), or right-clicks a sea to buy as many ships as it can pay for (from the selected adjacent territory's troops when one is selected, otherwise from the pool).
- **Purpose:** spend `ships * 3` troops and add `ships` to the caller's ship count in `seaTerritoryId`. The `3`-per-ship cost is either deducted from the caller's still-unspent `troopsToDeploy` (`fromPool: true`) or straight off `sourceTerritoryId`'s own troop stack, leaving at least `1` behind (`fromPool: false`); either way, `sourceTerritoryId` must be owned by the caller and directly adjacent to `seaTerritoryId`. Doesn't itself end `'deploy'` or advance the turn, but when paid `fromPool` and this leaves `troopsToDeploy` at `0`, the same auto-advance check `game:deploy` makes applies (see above). The caller must be the player at `turnPlayerIndex`, and the game must be `playing` and in `'deploy'`.
- **Content:**
  ```ts
  { sourceTerritoryId: number; seaTerritoryId: number; ships: number; fromPool: boolean }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not deploy phase`, `invalid territory`, `invalid sea`, `invalid ships`, `invalid payment`, `territory not owned`, `sea not connected to source`, `not enough troops`.
...
- **Purpose:** set (or clear, with `territoryId: null`) `fortifyStartTerritoryId`, clearing `fortifyEndTerritoryId` in the same call. A candidate start territory must be owned by the caller and have at least 2 troops; under `'Neighboring'` it must additionally have at least one neighboring territory also owned by the caller, under `'Connected'` it must be able to reach some other owned territory through the same chain described below (land, or extended across ship-held seas, see "Seas and ships" above), while `'Unrestricted'` only requires the caller to own some other territory. The client computes candidacy itself (for highlighting/hover), but the server independently re-checks it here. The caller must be the player at `turnPlayerIndex`, and the game must be `playing` and in `'fortify'`.
- **Content:**
  ```ts
  { territoryId: number | null }
  ```
...
- **Purpose:** set `fortifyEndTerritoryId`. A candidate end territory must be owned by the caller and different from `fortifyStartTerritoryId`; which ones qualify depends on `fortification` (see `GameState.fortification` above): reachable through a chain of territories all owned by the caller under `'Connected'` (a chain that may also cross any sea holding at least one of the caller's own ships, then back onto owned land on its far side, and so on across as many further ship-held seas as it takes; see "Seas and ships" above), a direct neighbor of the start territory under `'Neighboring'` (never across a sea, regardless of ships), or any other territory the caller owns under `'Unrestricted'`. As with the start territory, the client computes this for highlighting and the server re-checks it. The caller must be the player at `turnPlayerIndex`, and the game must be `playing` and in `'fortify'`.
- **Content:**
  ```ts
  { territoryId: number }
  ```
...
- **Purpose:** move `troops` from `fortifyStartTerritoryId` to `fortifyEndTerritoryId`, then immediately advance the turn phase (same as completing `'fortify'` via `game:nextPhase`, see `roundNumber` above); landing on `'entrench'` or `'toxins'` if either is enabled and the player has a legal move in it, otherwise ending the turn right away; since only one movement is allowed per fortify phase. The caller must be the player at `turnPlayerIndex`, the game must be `playing` and in `'fortify'` with both fortify territories selected, and `troops` must be an integer from `1` up to the start territory's current troop count minus `1` (at least 1 troop always stays behind), and the move must not take `fortifyEndTerritoryId` above `1000000` troops.
- **Content:**
  ```ts
  { troops: number }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not fortify phase`, `no fortify selection`, `invalid troops`, `territory troop cap exceeded`.

### `game:quickFortify`
- **When sent:** the player whose turn it currently is, during `'fortify'` with `fortifyStartTerritoryId` already set (whether or not `fortifyEndTerritoryId` is set too, in which case this replaces it), right-clicks one of the candidate end territories on the map.
- **Purpose:** the two-step fortify in a single call: exactly `game:fortifySelectEnd` with `territoryId`, then `game:fortify` with `troops` equal to the start territory's troops - 1, capped so the end territory stays within `1000000` troops. Each step validates, mutates state, and emits its events exactly as the standalone call would.
- **Content:**
  ```ts
  { territoryId: number }
  ```
- **Ack:** shared Ack response carrying the final state. Errors: any `game:fortifySelectEnd` or `game:fortify` error.

### `game:sailSelectStart`
- **When sent:** the player whose turn it currently is, during `'sail'`, clicks one of their candidate seas on the map, or cancels the in-progress sail selection (clicking/right-clicking outside it, or pressing Escape).
- **Purpose:** set (or clear, with `territoryId: null`) `sailStartTerritoryId`, clearing `sailEndTerritoryId` in the same call. A candidate start sea must hold at least 1 of the caller's ships. The caller must be the player at `turnPlayerIndex`, and the game must be `playing` and in `'sail'`.
- **Content:**
  ```ts
  { territoryId: number | null }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not sail phase`, `invalid sea`, `no ships there`.

### `game:sailSelectEnd`
- **When sent:** the player whose turn it currently is, during `'sail'` with `sailStartTerritoryId` already set, clicks one of the candidate end seas on the map.
- **Purpose:** set `sailEndTerritoryId`. A candidate end sea must be reachable from `sailStartTerritoryId` through a path of seas alone (ownership is meaningless here, seas have none). The caller must be the player at `turnPlayerIndex`, and the game must be `playing` and in `'sail'`.
- **Content:**
  ```ts
  { territoryId: number }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not sail phase`, `no start sea selected`, `invalid sea`, `invalid end sea`.

### `game:sail`
- **When sent:** the player whose turn it currently is, during `'sail'` with both `sailStartTerritoryId` and `sailEndTerritoryId` set, confirms the ship movement from the sail panel (its confirm button, or Enter).
- **Purpose:** move `ships` of the caller's own from `sailStartTerritoryId` to `sailEndTerritoryId`, then immediately advance the turn phase to `'attack'`, since only one movement is allowed per sail phase (unlike `'fortify'`, every ship may move: none need stay behind). The caller must be the player at `turnPlayerIndex`, the game must be `playing` and in `'sail'` with both seas selected, and `ships` must be an integer from `1` up to the caller's ship count on the start sea.
- **Content:**
  ```ts
  { ships: number }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not sail phase`, `no sail selection`, `invalid ships`.

### `game:quickSail`
- **When sent:** the player whose turn it currently is, during `'sail'` with `sailStartTerritoryId` already set (whether or not `sailEndTerritoryId` is set too, in which case this replaces it), right-clicks one of the candidate end seas on the map.
- **Purpose:** the two-step sail in a single call: exactly `game:sailSelectEnd` with `territoryId`, then `game:sail` with `ships` equal to all of the caller's ships on the start sea. Each step validates, mutates state, and emits its events exactly as the standalone call would.
- **Content:**
  ```ts
  { territoryId: number }
  ```
- **Ack:** shared Ack response carrying the final state. Errors: any `game:sailSelectEnd` or `game:sail` error.
...
### `game:buildNuke` / `game:buildAntiNuke`
- **When sent:** the player whose turn it currently is, during `'deploy'`, from the nukes panel, to start a new nuke or anti-nuke project (paying its first of `5` installments).
- **Purpose:** deduct the installment cost from `troopsToDeploy` (`5` for a nuke, `3` for an anti-nuke; those troops are never placed on the map) and append a new project to the caller's `nukeProjects` with `installmentsPaid: 1` and `lastPaidRound` set to the current `roundNumber`. Does not end the phase, though a `troopsToDeploy` of `0` afterward auto-advances it exactly as `game:deploy` does. The caller must be the player at `turnPlayerIndex`, the game must be `playing` and in `'deploy'`, `nukes` must be `'on'`, and `troopsToDeploy` must be at least the cost.
- **Content:** `{}`
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `nukes disabled`, `not your turn`, `not deploy phase`, `not enough troops`.

### `game:advanceNuke`
- **When sent:** the player whose turn it currently is, during `'deploy'`, from the nukes panel, to pay the next installment on one of their in-progress projects.
- **Purpose:** deduct the project's installment cost from `troopsToDeploy`, increment its `installmentsPaid`, and set its `lastPaidRound` to the current `roundNumber`. On reaching `5` installments the project is removed from `nukeProjects` and a nuke or anti-nuke is added to the caller's `arsenal`. Rejected if the project was already advanced this turn (`lastPaidRound >= roundNumber`), which is what bounds a device to `5` turns minimum. `index` is the project's position in the caller's `nukeProjects` array.
- **Content:**
  ```ts
  { index: number }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `nukes disabled`, `not your turn`, `not deploy phase`, `invalid project`, `no such project`, `already advanced this turn`, `not enough troops`.

### `game:launchNuke`
- **When sent:** the player whose turn it currently is, during `'attack'`, after arming "Launch nuke" in the nukes panel and clicking a target territory on the map.
- **Purpose:** consume one nuke from the caller's `arsenal` and fire it at `territoryId`. The rocket's origin is the caller's highest-troop capital, or (if the game has no capitals) their highest-troop territory. If the target's owner has an anti-nuke on any territory connected through their own territories to the target, that anti-nuke is consumed and the nuke is voided; otherwise the target is removed from `territoryOwners` with all its troops and entrenchment (capital/toxin/radiation markers left intact), possibly eliminating its owner. Either way the server broadcasts `game:nukeLaunched` to everyone and records a `'nuke'` `ReplayFrame`. `territoryId` may be any real territory that isn't owned by the caller. The caller must be the player at `turnPlayerIndex`, the game must be `playing` and in `'attack'`, `nukes` must be `'on'`, and the caller must have a nuke in their arsenal.
- **Content:**
  ```ts
  { territoryId: number }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `nukes disabled`, `not your turn`, `not attack phase`, `invalid territory`, `no nuke available`, `cannot nuke your own territory`, `no launch site`.

### `game:deployAntiNuke`
- **When sent:** the player whose turn it currently is, during `'attack'`, after arming "Deploy anti-nuke" in the nukes panel and clicking one of their own territories on the map.
- **Purpose:** consume one anti-nuke from the caller's `arsenal` and add `territoryId` to `antiNukeTerritoryIds`. It stays bound to that territory (ownership follows the territory) and is single-use. The caller must be the player at `turnPlayerIndex`, the game must be `playing` and in `'attack'`, `nukes` must be `'on'`, `territoryId` must be owned by the caller and not already hold an anti-nuke, and the caller must have an anti-nuke in their arsenal.
- **Content:**
  ```ts
  { territoryId: number }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `nukes disabled`, `not your turn`, `not attack phase`, `invalid territory`, `territory not owned`, `territory already protected`, `no anti-nuke available`.
...
- **Purpose:** set (or clear, with `territoryId: null`) `attackStartTerritoryId`, clearing `attackEndTerritoryId` and `attackConquestMinTroops` in the same call. A candidate attacking territory must be owned by the caller, have more than 1 troop, and either have at least one neighboring territory owned by a different player or unowned and not currently toxined (a free-conquest target, see the `toxins` paragraph above), or have a sea bridge (see "Seas and ships" above) to some such territory; the client computes candidacy itself (for highlighting/hover), but the server independently re-checks it here. The caller must be the player at `turnPlayerIndex`, the game must be `playing` and in `'attack'`, and no conquest may currently be pending. Starting a land attack selection (a non-`null` `territoryId`) is refused with `sea attack in progress` while `attackSeaTerritoryId` is set, so a land attack and a ship fight can never be selected at the same time.
- **Content:**
  ```ts
  { territoryId: number | null }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not attack phase`, `pending conquest move`, `invalid territory`, `territory not owned`, `invalid start territory`, `sea attack in progress`.
...
- **Purpose:** set `attackEndTerritoryId`, and compute the blitz win probability of every troop count the attack panel can offer, all in one response so the client never has to ask again while the panel is open. Regular attacks (1–3 troops) have no probability computed or sent: a regular attack is a single dice exchange with no meaningful "chance of winning the territory" the way a blitz has, so the panel shows no percentage for those options. A candidate defending territory must be either a neighbor of `attackStartTerritoryId` or reachable from it by a sea bridge (see "Seas and ships" above), and either be owned by a different player than the caller, or be unowned and not currently toxined (a free-conquest target, see the `toxins` paragraph above); the client computes this for highlighting, the server re-checks it here. The caller must be the player at `turnPlayerIndex`, and the game must be `playing` and in `'attack'` with a start territory already selected.
- **Content:**
  ```ts
  { territoryId: number }
  ```
- **Ack:** unlike other calls, this one is not the shared Ack response: it carries the computed probabilities alongside it:
  ...
  | {
      ok: true;
      game: GameState;
      blitzWinProbabilities: number[]; // index i = probability of winning with i+1 troops via blitz (trueWinProb or balancedWinProb, chosen by the game's blitz setting), length attacking territory's troops − 1. In 'Fair' blitz each entry is 1 or 0: whether committing i+1 troops conquers the territory. In 'Off' blitz every entry is 0 and only the length is meaningful (it still tells the client how many troops can attack).
...
- **Ack:** like `game:attackSelectEnd`, not the shared Ack response: it carries fresh blitz win probabilities (empty when the battle was conquered, or when the attacking territory can no longer attack) plus the dice actually rolled (both empty for `type: 'blitz'`, since blitz doesn't animate a roll):
  ...
  | { ok: false; error: string }
  ```
  Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not attack phase`, `no attack selection`, `territory already conquered`, `invalid attack type`, `blitz disabled`, `invalid troops`.
...
### `game:quickAttack`
- **When sent:** the player whose turn it currently is, during `'attack'` with `attackStartTerritoryId` already set (whether or not `attackEndTerritoryId` is set too, in which case this replaces it), right-clicks one of the candidate defending territories on the map. Never sent when the game's `blitz` setting is `'Off'`.
- **Purpose:** the three-step attack in a single call: exactly `game:attackSelectEnd` with `territoryId`, then `game:attack` with `{ type: 'blitz', troops: <attacking territory's troops - 1> }`, then, if that conquers the territory and the game is still `playing`, `game:attackMove` with `troops` equal to the attacking territory's remaining troops - 1. Each step validates, mutates state, and emits its events (`game:selected`, `game:attacked`, `game:tankFired`, `game:attackMoved`, `game:state` to the other sockets) exactly as the standalone call would, so every client animates it the same way. A failed blitz leaves the attacking territory with `1` troop, which resets both attack territory ids as described under `game:attack`. Rejected up front, before any state changes, when blitz is `'Off'` or a conquest is already pending.
- **Content:**
  ```ts
  { territoryId: number }
  ```
- **Ack:** shared Ack response carrying the final state. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not attack phase`, `blitz disabled`, `pending conquest move`, plus any `game:attackSelectEnd` error.

### `game:attackSeaSelectStart`
- **When sent:** the player whose turn it currently is, during `'attack'`, clicks a sea where they have ships alongside another player's, or cancels the in-progress ship-fight selection.
- **Purpose:** set (or clear, with `territoryId: null`) `attackSeaTerritoryId`, clearing `attackSeaDefenderId` in the same call. A candidate sea must hold at least 1 of the caller's ships (whether or not another player's ships are present is left to `game:attackSeaSelectDefender` to enforce, since a client may not yet know who to attack). The caller must be the player at `turnPlayerIndex`, and the game must be `playing` and in `'attack'`. Starting a ship-fight selection (a non-`null` `territoryId`) is refused with `land attack in progress` while `attackStartTerritoryId` is set (including mid-conquest), so a land attack and a ship fight can never be selected at the same time.
- **Content:**
  ```ts
  { territoryId: number | null }
  ```
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not attack phase`, `invalid sea`, `no ships there`, `land attack in progress`.

### `game:attackSeaSelectDefender`
- **When sent:** the player whose turn it currently is, during `'attack'` with `attackSeaTerritoryId` set, picks which other player's ships there to attack (the sea's defender-picker panel; skipped automatically when only one other player has ships there).
- **Purpose:** set `attackSeaDefenderId`, and compute the blitz win probability of every ship count the attack panel can offer, all in one response, mirroring `game:attackSelectEnd` for land attacks — except the range spans the caller's full ship count there rather than count − 1, since a ship fight has no territory to hold back a reserve for (see `game:attackSea` below). `defenderId` must be a different player with at least 1 ship on `attackSeaTerritoryId`. The caller must be the player at `turnPlayerIndex`, and the game must be `playing` and in `'attack'`.
- **Content:**
  ```ts
  { defenderId: number }
  ```
- **Ack:** unlike other calls, this one is not the shared Ack response: it carries the computed probabilities alongside it, the same shape `game:attackSelectEnd` uses:
  ```ts
  | {
      ok: true;
      game: GameState;
      blitzWinProbabilities: number[]; // index i = probability of winning with i+1 ships via blitz, length = caller's ship count on attackSeaTerritoryId. In 'Off' blitz every entry is 0 and only the length is meaningful
      blitzOutcomes?: { attackLosses: number; defenceLosses: number }[]; // only present when blitz is 'Fair'; index i = the predetermined losses of committing i+1 ships, same length as blitzWinProbabilities
    }
  | { ok: false; error: string }
  ```
  Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not attack phase`, `no sea selected`, `invalid defender`.

### `game:attackSea`
- **When sent:** the player whose turn it currently is, during `'attack'` with both `attackSeaTerritoryId` and `attackSeaDefenderId` set, confirms a ship-fight attack option from the attack panel.
- **Purpose:** resolve a fight between the caller's and `attackSeaDefenderId`'s ships on `attackSeaTerritoryId`, via the same dice-based combat as a land attack (see "Seas and ships" above) except a tie favors neither side. For `type: 'regular'`, `ships` (1 up to `min(caller's ships there, 3)`) fight exactly one exchange via `attackSea()` in `dice.ts`, and the raw dice results are returned (see Ack below) so the client can animate the roll; the defender rolls `min(its ships there, defenceDice)`. For `type: 'blitz'`, `ships` (1 up to the caller's full ship count there — no reservation, since there's no territory to hold) fight to elimination of one side via `trueBlitz()`, `balancedBlitz()`, or `fairBlitz()` (chosen by the game's `blitz` setting, exactly like a land blitz), except every dice exchange along the way still treats a tie as neutral rather than favoring the defender. As with land, `'Fair'` blitz never rolls dice: the losses are fixed in advance from the ship counts and defence dice, the side favoured by the exact odds wins, and the loser is wiped. Losses are applied immediately to both sides' ship counts; a side reaching `0` ships there is removed from `seas` entirely. If either side is left with `0` ships there, `attackSeaTerritoryId`/`attackSeaDefenderId` reset to `null`, ending this fight (same "no ships left to continue" rule fortify/attack territories follow); otherwise both stay set and blitz win probabilities are recomputed for the reduced ship counts, mirroring `game:attack`. Unlike `game:attack`, there is no conquest and no follow-up move. The caller must be the player at `turnPlayerIndex`, the game must be `playing` and in `'attack'` with both selected, and the defender must still have at least 1 ship there. When the game's `blitz` setting is `'Off'`, `type: 'blitz'` is always rejected (`blitz disabled`).
- **Content:**
  ```ts
  { type: 'regular'; ships: number } | { type: 'blitz'; ships: number }
  ```
- **Ack:** like `game:attack`, not the shared Ack response: it carries fresh blitz win probabilities (empty when either side was left with `0` ships) plus the dice actually rolled (both empty for `type: 'blitz'`, since blitz doesn't animate a roll):
  ```ts
  | {
      ok: true;
      game: GameState;
      blitzWinProbabilities: number[];
      blitzOutcomes?: { attackLosses: number; defenceLosses: number }[]; // only present when blitz is 'Fair' and the fight stays open; see `game:attackSeaSelectDefender`
      attackerDice: number[];
      defenderDice: number[];
    }
  | { ok: false; error: string }
  ```
  Errors: `not in a game`, `game not found`, `game not started`, `game paused`, `not your turn`, `not attack phase`, `no attack selection`, `invalid attack type`, `blitz disabled`, `invalid ships`, `invalid defender`.

### `game:quickAttackSea`
- **When sent:** the player whose turn it currently is, during `'attack'` with no land attack or ship fight selected, right-clicks a sea where they have ships alongside exactly one other player's. Never sent when the game's `blitz` setting is `'Off'`. When more than one other player has ships there, the client sends `game:attackSeaSelectStart` instead, so the defender is picked from the panel as usual.
- **Purpose:** the three-step ship fight in a single call: exactly `game:attackSeaSelectStart` with `territoryId`, then `game:attackSeaSelectDefender` with the only other player that has ships there, then `game:attackSea` with `{ type: 'blitz', ships: <all of the caller's ships there> }`. Each step validates, mutates state, and emits its events exactly as the standalone call would. The blitz always ends the fight, so both ship-fight ids are reset afterwards.
- **Content:**
  ```ts
  { territoryId: number }
  ```
- **Ack:** shared Ack response carrying the final state. Errors: `blitz disabled`, `ambiguous defender` (not exactly one other player has ships there, leaving the sea selected as `game:attackSeaSelectStart` would), plus any `game:attackSeaSelectStart`, `game:attackSeaSelectDefender`, or `game:attackSea` error.
...
The game list is no longer pushed — the client polls `GET /games/live` while the home tab is visible (see "HTTP API"). Everything below is sent only to a game's socket.
...
- **Content:**
  ...
  ```
  `elo` / `eloDelta` (the player's elo before the game and the change this game applied, `0` for a game with fewer than two accounts) are present only for registered accounts, and only once the server has computed them. That happens right after the game ends, so `game:results` is sent twice at game end: first without them, then again, to everyone in the room, once they are known. Later sends (reconnect, `game:requestResults`) include them while the game is still alive on the server.
  One entry per `id` in `GameState.players[]`. `userId` is the server-only account lookup for that player id (`null` for a bot or an anonymous player); it rides here rather than on `GameState.players[]` since, like the rest of this payload, it's irrelevant until the results table after the game ends, and this is the one event guaranteed to reach every client while its sockets are still connected. An offline game's engine-only `game:results` (no server in the loop) omits `userId` entirely.
...
### `game:shipsBought`
- **When sent:** immediately, individually to every socket in a game's room whenever a `game:buyShips` call succeeds (including back to the buying player). While `fogOfWar` is in effect for a given recipient, this event is withheld entirely from them unless at least one of `sourceTerritoryId`/`seaTerritoryId` is currently visible to them.
- **Purpose:** let every client play the deploy sound effect and animate the new ships appearing at `seaTerritoryId`, since `GameState` is a snapshot with nothing to infer this from. `playerId` is the buying player; `fromPool` tells the client whether `sourceTerritoryId`'s own troop count also dropped by `ships * 3` (so it knows whether to animate a matching deduction there) or the cost came out of the caller's still-unspent `troopsToDeploy` pool instead (invisible to other recipients either way).
- **Content:**
  ```ts
  { sourceTerritoryId: number; seaTerritoryId: number; ships: number; fromPool: boolean; playerId: number }
  ```
...
### `game:sailed`
- **When sent:** immediately, individually to every socket in a game's room whenever a `game:sail` call succeeds (including back to the moving player), or the turn timer force-completes a pending `'sail'` move. While `fogOfWar` is in effect for a given recipient, this event is withheld entirely from them unless at least one of `seaTerritoryId`/`fromSeaTerritoryId` is currently visible to them; like `game:fortified`, the ship-count fields are redacted field-by-field (see `shipsRemoved`/`shipsAdded` below).
- **Purpose:** let every client play the fortify-style movement animation between the two seas. `playerId` is the acting player; `path` is the sea-to-sea path actually walked, split into separate runs the same way `GameState.fortifyPathTerritoryIds` is wherever a hop's both endpoints are invisible to the recipient. `shipsRemoved`/`shipsAdded` follow the `game:fortified` rule exactly: the same moved amount, included only for whichever of `fromSeaTerritoryId`/`seaTerritoryId` is visible to this recipient.
- **Content:**
  ```ts
  { seaTerritoryId: number; fromSeaTerritoryId: number; playerId: number; path: number[][]; shipsRemoved?: number; shipsAdded?: number }
  ```
...
### `game:nukeLaunched`
- **When sent:** immediately, to every socket in a game's room (and every spectator), whenever a `game:launchNuke` call resolves — whether the nuke landed or was intercepted. Never filtered by `fogOfWar`: it carries only territory ids and public elimination data, so every client sees and hears the rocket and the blast even for a target hidden by fog (the territory's actual ownership change still rides the normal fog-filtered `game:state`).
- **Purpose:** let every client fly the rocket from `fromTerritoryId` to `targetTerritoryId` and play the mushroom-cloud explosion (reusing the explosion sound) in sync; a `'nuke'` `ReplayFrame` carries the same payload for replays. `playerId` is the launcher. When `intercepted` is `true`, `interceptFromTerritoryId` is the anti-nuke territory that fired the interceptor (a smaller rocket meeting the incoming one, a smaller explosion, no obliteration); when `false` it is `null` and the target was obliterated. `eliminatedPlayerIds` lists any player whose last territory this nuke took, in no particular order; empty most of the time and never filtered, same as `game:radiationChanged`.
- **Content:**
  ```ts
  { playerId: number; fromTerritoryId: number; targetTerritoryId: number; intercepted: boolean; interceptFromTerritoryId: number | null; eliminatedPlayerIds: number[] }
  ```
...
- **Purpose:** let every client play the fortify sound effect and the deploy animation on the newly-conquered territory in sync, since `GameState` is a snapshot, not a record of individual actions, so a client has nothing to infer this from even once its next push arrives. `troopsRemoved`/`troopsAdded` follow exactly the `game:fortified` rule above: the same moved amount, included only for whichever of `fromTerritoryId`/`territoryId` is visible to this recipient. `path` follows exactly the `game:attacked` rule above (the same conquest, so the same bridging sea territory if any), letting the client draw this follow-up move's arrow through it too.
- **Content:**
  ```ts
  { territoryId: number; fromTerritoryId: number; path: number[][]; troopsRemoved?: number; troopsAdded?: number }
  ```

### `game:seaAttacked`
- **When sent:** immediately, individually to every socket in a game's room whenever a `game:attackSea` call succeeds (including back to the attacking player). While `fogOfWar` is in effect for a given recipient, this event is withheld entirely from them unless `seaTerritoryId` is currently visible to them, or the recipient is the defender themselves (ships are removed from the sea before this event is computed, which can otherwise cost the defender their only source of visibility into it; they always learn the outcome of a fight over their own ships regardless, mirroring the `defenderId`-carve-out on the land `game:attacked` event above); unlike that land event, there's only the one territory to check, so nothing here is redacted field-by-field.
- **Purpose:** let every client play the attack sound/dice animation for the ship fight, since `GameState` is a snapshot with nothing to infer this from. `attackingShips`/`defendingShips` are the ships each side committed to this exchange (before losses); `attackLosses`/`defenceLosses` are what each side lost. `type` echoes the `game:attackSea` call's own `type`, the same way `game:attacked`'s `type` does for land: the client uses it to skip the dice-roll animation and land the loss instantly for `type: 'blitz'`, since blitz never actually rolls.
- **Content:**
  ```ts
  { seaTerritoryId: number; attackerId: number; defenderId: number; type: 'regular' | 'blitz'; attackingShips: number; attackLosses: number; defendingShips: number; defenceLosses: number }
  ```

### `game:selected`
- **When sent:** immediately, to every socket in a game's room, whenever a `game:selectTerritory`, `game:fortifySelectStart`, `game:fortifySelectEnd`, `game:attackSelectStart`, `game:attackSelectEnd`, `game:sailSelectStart`, `game:sailSelectEnd`, or `game:attackSeaSelectStart` call succeeds with a non-`null` `territoryId` (including back to the selecting player). Not sent for a deselection. Never filtered by `fogOfWar`: it carries only a territory id, no ownership or troop data.

...
```
`radiationUpcoming`, `hands`, and `turnPhase` are full snapshots taken at the moment of the frame (all cheap, unlike `territories`), so a replay's scoreboard can track card hands, troop counts, and the round/phase as they evolve.

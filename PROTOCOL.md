...
- **session token**: a high-entropy random string **generated by the server**. It lives only in the `anx` cookie, which the **server** sets on the Socket.IO handshake response as `HttpOnly; SameSite=Lax` (plus `Secure` when the handshake is HTTPS, or is reported as HTTPS by a proxy the server is configured to trust) — client JavaScript never reads or writes it, and it never appears in any message payload or ack. The browser returns it automatically on every connection (the client socket uses `withCredentials`, and the server CORS allows credentials for the one configured origin — client and server must therefore be same-site in production; the server only trusts `X-Forwarded-For` / `X-Forwarded-Proto` when `TRUST_PROXY_HOPS` is set to the number of proxies in front of it, otherwise it uses the direct connection). It's a secret credential: whoever presents a given token is treated as that player, and any other socket already holding that identity is disconnected. Every connection has one, logged in or not:
  - **anonymous**: the token maps, in server memory only (never the database), to the player's `id`. The server mints it in the handshake when the cookie is absent; a token it doesn't recognise on a later connection (e.g. after a restart wiped memory) just gets a fresh anonymous `id` bound to it.
  - **logged in**: the token is stored **hashed** (SHA-256) in the `sessions` collection alongside the account's user id and a `createdAt`. Two limits apply: a rolling `expireAt` (30 days, slid forward on every `player:identify` that presents the token — so idle inactivity kills it after a month) and an absolute `createdAt` cap (180 days, after which the session is dead no matter how active). Both are enforced on read and backed by TTL indexes. The token is **rotated on every login** (see "Accounts"): the pre-login token is discarded and a fresh one issued, so a value observed before authentication cannot be used after it.
...
An anonymous player, once seen, stays in server memory (keyed by `id` and token) for as long as the server runs, connected or not. A logged-in player's account (username, email, hashed password, saved settings) is persisted; their in-memory player is rebuilt from the `sessions` record after a restart, keyed to the same account.

### Accounts

Accounts are optional; both anonymous and logged-in clients can play identically. An account has a **username** (unique case-insensitively) and an **email** (unique after normalisation: lowercased, and dots stripped from the local part so Gmail-style aliases collapse), neither ever changeable, a hashed password (Argon2id), and a `validated_email` flag. Registration (`auth:register`) creates the account unvalidated and emails a confirmation link pointing at the client (`.../email_confirmation/<code>`); the client opens a socket and sends the code via `auth:confirmEmail`, which confirms the email but does **not** log the socket in — the user logs in normally afterwards. Plain `auth:login` is refused until the email is confirmed (and re-sends a fresh confirmation code). Logging in — via `auth:login` or `auth:resetPassword` — mints a **fresh** session token, stores it hashed in the `sessions` collection, discards the pre-login token, and adopts the account's in-memory player, so a client mid-lobby keeps its seat and a client whose account is mid-game is pulled into that game (the ack's `gameName`); the client then reconnects its socket so the browser receives the rotated `anx` cookie on the new handshake. Both carry a `stayLoggedIn` flag (default true); when false the rotated `anx` cookie is a session cookie (dropped when the browser closes) rather than the usual multi-year persistent cookie — the `sessions` row itself is unaffected. A player is free to log in from `home`, a lobby, or the results screen (there is no login form inside an active game's screen). `auth:logout` surrenders any `playing` game first, then deletes the `sessions` row and demotes the same cookie token back to a fresh anonymous identity with a random name. Because a `playing`-game seat is pinned to the identity and the identity is the account (or, for an anonymous client, the cookie's session token), a client with a game in progress cannot navigate away from it or start another: `player:identify` keeps reporting the real `gameName` and the client is redirected back (except on the `/email_confirmation/` and `/password_reset/` pages, which stay put so the flow can finish). This holds across a logged-in account's tabs and devices, and across an anonymous client's tabs in the same browser. After a failed `auth:login`, the client offers two email-based recovery paths: `auth:recoverUsername` (emails the username) and `auth:requestPasswordReset` → emailed `.../password_reset/<code>` link → `auth:resetPassword` (which also logs the socket in). All `auth:*` events are rate-limited per client IP; `auth:login` additionally answers `too many requests` after 8 wrong-password attempts on one username within 15 minutes, though the real owner (correct password) is never blocked.
...
Two settings blobs travel in the `player:identify` / login / logout acks: **client settings** (`muted`, `volume`, `animationsDisabled`) and **game settings** (the remembered lobby setup: `mapName`, `mapGeneration`, `slots`, and every `game:settings` rule field). For a logged-in client both are persisted in the account and pushed back with `user:updateSettings`. For an anonymous client nothing is persisted: it always receives the server defaults and its `user:updateSettings` calls are ignored.

### Leaving and reconnecting

Closing the tab, opening a new one, refreshing, or otherwise dropping the connection all disconnect the socket. Reconnecting (a fresh `player:identify`) with the same session token resumes the same `id`; a missing or unrecognised token is a different player: a new `id` who, per the seating rules below, can only ever join as a spectator.

Logging in triggers its own deliberate socket reconnect (to pick up the rotated `anx` cookie, see "Player identity" above). This one is transparent: for the brief drop the server keeps the player's `connected` state and game membership untouched — no stand-in, no bot takeover, no roster change, nothing broadcast to other clients — and the immediate reconnect resumes exactly where it left off. If that reconnect never arrives (~5s), the drop is then treated as an ordinary disconnect.
...
- **Content:**
  ...
  { room: string }
  ```
  No token is sent: the server reads the `anx` cookie off the connection's handshake (see "Player identity" above). It resolves the token: matches a `sessions` row → that logged-in account (slides its TTL, enforces the absolute cap); held in memory → that anonymous player; anything else → a brand-new anonymous player. The client never sends a name; the server assigns it (`PlayerNNNN` anonymous, username logged-in).
- **Ack:**
  ```ts
  {
    id: number;
    ...
    clientSettings: { muted: boolean; volume: number; animationsDisabled: boolean };
    gameSettings: { mapName: string; mapGeneration: {...} | null; slots: number; /* + every game:settings rule field */ };
  }
  ```
  `id` is the caller's own id, stable for as long as the player exists on the server. `name` is the current display name (`PlayerNNNN` for an anonymous client, the username when logged in); the client shows it in the account panel. `account` is `null` for anonymous clients. `clientSettings`/`gameSettings` are the account's saved values when logged in, otherwise the server defaults. `gameName` is the player's actual game membership after the server resolved this identify (see "Leaving and reconnecting" above), or `null` if they're not seated/spectating anywhere. A seated, not-surrendered player in a `playing` game can never shed this by declaring `room: 'home'` (or any other game's name): the server keeps their membership and reports the real `gameName` back regardless of what `room` was requested, and the client always navigates to whatever `gameName` the ack reports when it differs from the requested `room`, forcing them back to their game (the `/email_confirmation/` and `/password_reset/` pages are exempt — they ignore the redirect until their flow finishes). Surrendering, or simply never reconnecting after closing the tab, are the only ways to actually leave a `playing` game early; once `state` is `'ended'`, a mismatched-room identify clears membership as normal (see "Leaving and reconnecting" above), so leaving then works as requested.

### `auth:register`
- **When sent:** a client submits the create-account form.
- **Content:** `{ username: string; email: string; password: string }` — username 3–10 chars matching `^[A-Za-z0-9_-]+$` and not profane, email ≤50 chars, password 8–128 chars.
- **Purpose:** create the account with `validated_email: false` and email a confirmation link to `.../email_confirmation/<code>` (code TTL 24h). Does **not** log the client in. If the (normalised) email is already registered, no account is created and `{ ok: true }` is still returned, but the mail sent to that address says an account already exists — so the ack never reveals whether an email is taken.
- **Ack:** `{ ok: true } | { ok: false; error: string }`. Errors: `invalid username`, `invalid email`, `invalid password`, `username already taken`, `too many requests`, `server error`. `{ ok: true }` means "check your email", not necessarily "account created".

### `auth:confirmEmail`
- **When sent:** the `/email_confirmation/<code>` client page loads and opens a socket.
- **Content:** `{ code: string }`
- **Purpose:** mark the account's `validated_email` true and delete the confirmation. Does **not** log the socket in — the user logs in with `auth:login` afterwards.
- **Ack:** `{ ok: true } | { ok: false; error: string }`. Errors: `invalid code`, `invalid or expired code`, `too many requests`, `server error`.

### `auth:recoverUsername`
- **When sent:** a client clicks "Forgot username?" (shown only after a failed `auth:login`) and submits its email.
- **Content:** `{ email: string }`
- **Purpose:** if an account has that email (matched after normalisation), send its username to that address.
- **Ack:** always `{ ok: true }` — never reveals whether the email has an account (also returned, doing nothing, when the caller's IP is rate-limited).

### `auth:requestPasswordReset`
- **When sent:** a client clicks "Forgot password?" (shown only after a failed `auth:login`) and submits its email.
- **Content:** `{ email: string }`
- **Purpose:** if an account has that email (matched after normalisation), email a reset link to `.../password_reset/<code>` (code TTL 1h, replacing any prior reset code).
- **Ack:** always `{ ok: true }` — never reveals whether the email has an account (also returned, doing nothing, when the caller's IP is rate-limited).

### `auth:resetPassword`
- **When sent:** the `/password_reset/<code>` client page submits a new password.
- **Content:** `{ code: string; password: string }` — password 8–128 chars.
- **Purpose:** set the new password hash, mark `validated_email` true (clicking the emailed link proves ownership), delete the reset code, and delete every existing `sessions` row for that account. Does **not** log the socket in — the user logs in with `auth:login` afterwards, so a browser already logged into another account is left untouched.
- **Ack:** `{ ok: true } | { ok: false; error: string }`. Errors: `invalid code`, `invalid password`, `invalid or expired code`, `too many requests`, `server error`.

### `auth:login`
- **When sent:** a client submits the login form (from `home`, a lobby, or the results screen). The socket must already be identified.
- **Content:** `{ username: string; password: string; stayLoggedIn?: boolean }` — `stayLoggedIn` defaults true; when false the rotated `anx` cookie is written without `Max-Age` (a session cookie the browser drops on close) instead of the multi-year persistent cookie.
- **Purpose:** verify the password (case-insensitive username match), then mint a fresh session token, store it hashed in the `sessions` collection, discard the pre-login token, and rename/keep the in-memory player as the account. If the account already has an active in-memory player (another tab or device), that one is adopted instead and its old socket is disconnected — so the identity, and any game it is in, follows the account. An unconfirmed account is refused and re-sent a fresh confirmation email. To defeat timing-based user enumeration the server always performs one password verification even when the username is unknown.
- **Ack:** `{ ok: true; username: string; clientSettings; gameSettings; gameName: string | null } | { ok: false; error: string }`. `gameName` is the game the caller ended up in (`null` if none); the client navigates there, so logging in while the account is mid-game pulls the client into that game. On `ok` the client reconnects its socket to receive the rotated `anx` cookie on the new handshake. Errors: `invalid credentials`, `email not confirmed`, `not identified`, `too many requests` (per-IP rate limit, or 8 wrong-password attempts on this username within 15 min — a login with the correct password always succeeds and clears that count), `server error`.

### `auth:logout`
- **When sent:** a logged-in client clicks "Log out".
- **Content:** none
- **Purpose:** if the in-memory player is in a `playing` game, **surrender** it first (leaving the account mid-game gives up that game). Then delete the `sessions` row and demote the same cookie token to a fresh anonymous identity (new random `PlayerNNNN` name, server-default settings) for the same in-memory player. The cookie itself is unchanged — it's just no longer account-backed.
- **Ack:** `{ ok: true; name: string; clientSettings; gameSettings } | { ok: false; error: string }` — `name` is the new `PlayerNNNN` display name; the settings blobs are the server defaults.

### `user:updateSettings`
- **When sent:** a client changes a client setting (sound, volume, animations) or saves its lobby setup, debounced.
- **Content:** `{ clientSettings?: {...}; gameSettings?: {...} }`
- **Purpose:** persist the settings on the logged-in account. Ignored (no-op) for an anonymous client.
- **Ack:** none

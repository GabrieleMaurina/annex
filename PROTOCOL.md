...
```
`territories` is a full snapshot (not a diff) of every territory's owner and troop count immediately after the change described by `animation`: `isCapital` isn't included, since it's a static property of the territory (see `turnPhase` above) that a client already has from `GameState.territories`. A game's replay is the sequence of these snapshots: the state right after `game:start` deals territories (no frame: nothing preceded it), then one `ReplayFrame` for every subsequent troop deployment, fortify, or attack for the rest of the game, in order. `'fortify'`-type frames cover both an actual `game:fortify` move and troops moving into a just-conquered territory (`game:attackMove`, or the turn timer completing either); a `'deploy'`-type frame covers `game:deploy`, a capital's placement troops, a card set's per-territory bonus, and the turn timer dropping leftover troops; one frame per territory actually touched, not per individual troop unit, even when several troops land on the same territory in one go. A successful attack that conquers its target always produces two separate, consecutive frames: the `'attack'` frame for the battle itself (troop losses and, if conquered, the ownership transfer), immediately followed by a `'fortify'` frame for however the resulting troop move happens: a later `game:attackMove`, or, if the attacker's hand hit 5+ cards or the conquest ended the game outright, one resolved automatically within the same turn. `turnNumber` is `GameState.turnNumber`'s value at the moment this frame happened (see `turnPlayerIndex` above for what it counts); `playerId` is whoever acted: the depositing/fortifying player, or the attacker for an `'attack'` frame.
...
```ts
{
  ...
  turnPhase: 'territory' | 'troop' | 'capital' | 'deploy' | 'attack' | 'fortify';
...
`connected` is whether the player currently has a live socket anywhere on the server (not necessarily this game's room, see "Leaving and reconnecting" above). `surrendered` is whether they've called `game:surrender` on this game; they're typically still `connected` (surrendering doesn't disconnect them) and keep their seat for good (see "Leaving and reconnecting" above), but are permanently skipped in turn order and rejected by every `game:*` action requiring their turn. `connected` is otherwise informational only: an absent, not-surrendered player's territories, troops, and turn are still tracked exactly like anyone else's. `connectedAtEnd` is a frozen snapshot of `connected` taken the moment `state` first becomes `'ended'`; unlike `connected`, it never changes afterward no matter how many times that player disconnects or reconnects, so an end-of-game UI can show who was actually still there when the game finished. Always `false` before the game has ended.
...
- **Purpose:** create a game with default settings: name `Game with <playerName>` unless `name` is given, map `World`, 2 slots, `Supremacy` game mode, `Balanced` blitz, 2 defence dice, `Constant` cards, `Random` placement, 120s turn duration; make the caller host and move their socket into the room.
- **Content:**
  ```ts
...
- **Purpose:** single bundled message for every settings mutation: rename, change map, slot count, ban list, a player's team, game mode / blitz / defence dice / cards / placement / fortification / turn duration. Only the fields present are applied; the caller must be host, and the game must still be `lobby`: nothing can change once `playing`. Fields apply in a fixed order: `mapName`, `gameMode`, `name`, `bannedPlayerIds`, `playerTeam`, `slots`, `blitz`, `defenceDice`, `cards`, `placement`, `fortification`, `turnDuration`, so `slots`/`playerTeam` are validated against the roster *after* any kicks in the same request. `gameMode` and the last six fields are independent of the rest and each other; their position otherwise doesn't matter.
...
- **Ack:** shared Ack response. Errors: `not in a game`, `game not found`, `not the host`, `game already started`, `invalid name`, `invalid map`, `invalid slots`, `invalid banned players`, `invalid team`, `invalid game mode`, `invalid blitz`, `invalid defence dice`, `invalid cards`, `invalid placement`, `invalid fortification`, `invalid turn duration`, `game name already in use`.
...
- **Purpose:** permanently give up the caller's ability to act in the game, and move their socket back to `home`. Slot, territories, and troops stay exactly as they were (same as any other player leaving a `playing` game, see "Leaving and reconnecting" above), so the game carries on unaffected, and the caller keeps rejoining as a player exactly as before; the only lasting difference from an ordinary disconnect is that they're now permanently skipped in turn order and every `game:*` action requiring their turn is rejected, so they can never again act on their troops or cards. If it was the caller's turn at the moment they surrender, that turn is force-ended immediately, exactly as `turnDuration` timing out would.
- **Content:** none
